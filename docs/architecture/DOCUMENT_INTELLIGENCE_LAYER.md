...
- **Cyrillic read by vision; Latin produced by deterministic KMU-55.** One place, never the LLM. Empirically proven (REDACTED / Serhiiovych / Trostianets correct; LLM's own "Troshchianets" rejected).
...
- Live (self-skips in CI; `RUN_LIVE_VISION=1`): `geminiVisionArbiter.live.test.ts` — real Gemini on owner booklet through the spine → REDACTED / Serhii / Serhiiovych / 1986-06-25 / Trostianets / Vinnytsia Oblast.
