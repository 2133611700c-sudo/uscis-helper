...
|---|---|---|---|---|---|---|
| **gemini-3.1-pro-preview** | preview | **PRIMARY LLM** | ❌ unreliable — 503 + 429; also run-to-run UNSTABLE (preview, ADR-026/HTR_STABLE_BENCHMARK) | PRINTED Cyrillic, as the acceptance LLM. NOT the best handwriting reader (key-free raxtemur beats it 3/6 vs 2/6 on cursive — ADR-026); NOT the handwriting reader | run-to-run instability; fabricates/wrong on cursive vs raxtemur; main failure is AVAILABILITY + STABILITY | D1 acceptance LLM for PRINTED fields. Handwriting → raxtemur (route by rendering, ADR-026) |
