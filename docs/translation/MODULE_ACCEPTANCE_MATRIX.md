...
| `ua_birth_certificate` | `birthCertificate.module.ts` | `active` | yes | yes | 14 | `birthCertificateValidators.ts` (43 tests) | yes (`civil_registry_terms.json`, era guards) | `birthCertificateExtractionPrompt.ts` | `birthCertificate.template.ts` (25 tests) | synthetic safe (no real PII): seeded session `658e664f-…` with 14 placeholder fields (TEST_FAKEFAMILY etc.), confirmed=true | 78 module + 43 validators + 25 template = 146 | **PASS** (synthetic E2E 2026-05-09 against deployed_commit `08753eb` — POST /certify 200 + POST /render 200 + application/pdf 3444 B / 2 pages) | **PASS** (18 forbidden phrases scanned against `artifacts/e2e/birth_cert/synthetic_smoke_text.txt`, 0 violations; 0 cyrillic chars in output) | **PASS** (3 audit_log rows: ocr_completed / certification_completed / final_rendered — metadata enums only, 0 of 12 PII markers found incl. names, dates, certificate numbers, signer email) | yes — falls back if classifier confidence <0.85 or fields missing | LOW: synthetic fixture only — no real-customer fixture committed (not allowed under no-real-PII rule, but adds replay risk if real OCR has unusual layouts). | Acquire a sanitized **real** birth-certificate fixture (public-domain sample or operator-provided test scan). Re-run the same smoke against it. If it passes, mark fixture as "live"; if it fails on any field, demote module to draft until fixed. Evidence: `artifacts/e2e/birth_cert/synthetic_smoke_summary.json`. |
...
### 3.2 `ua_birth_certificate` (active)

- ~~HIGH — no committed live OCR fixture, no E2E smoke, no PDF forbidden-phrase scan against a real birth certificate.~~ **CLOSED 2026-05-09 via synthetic E2E smoke** (`artifacts/e2e/birth_cert/synthetic_smoke_summary.json`). Verdict PASS: 14/14 critical fields present, render→PDF 3444B/2 pages, 0 forbidden-phrase violations across 18 phrases, 0 cyrillic chars in PDF text.
- ~~HIGH — no privacy QA pass against a generated birth-cert customer PDF (only against the passport one).~~ **CLOSED 2026-05-09**. 3 audit_log rows for the smoke session, 0 of 12 PII markers found in any metadata.
- MEDIUM — no live verification that the `civil_registry_terms` glossary correctly handles ЗАГС vs РАЦС vs ДРАЦС on a real Soviet-era certificate. Synthetic smoke used `'Kyiv Civil Registry Office'` placeholder; era-guard live test still pending.
- LOW — no real (sanitized) customer fixture committed yet. Synthetic-only fixture limits how much the smoke proves about actual OCR/extraction behaviour on Ukrainian birth certificates.
...
- Does **not** claim the active modules are ready for unlimited public launch. The honest read of the matrix:
  - **`ua_internal_passport_booklet`** is the only `active` module with full E2E + PDF + privacy evidence committed. Controlled pilot is appropriate; broad public launch is not.
  - **`ua_birth_certificate`, `ua_marriage_certificate`, `ua_divorce_certificate`** have unit-test coverage and PDF templates but **no committed live fixture / E2E / PDF QA**. They are `active` in code (and therefore can produce auto-PDFs in production), but the "active" status is **not yet evidenced** end-to-end. This is a real gap and is the next priority before any public marketing.
