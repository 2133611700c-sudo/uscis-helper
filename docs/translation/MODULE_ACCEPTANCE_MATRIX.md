...
| `ua_birth_certificate` | `birthCertificate.module.ts` | `active` | yes | yes | 14 | `birthCertificateValidators.ts` (43 tests) | yes (`civil_registry_terms.json`, era guards) | `birthCertificateExtractionPrompt.ts` | `birthCertificate.template.ts` (25 tests) | none committed (no live customer fixture; tests use synthetic) | 78 module + 43 validators + 25 template = 146 | NOT RUN (no E2E smoke yet — only the passport smoke from 2026-05-09 covers a different module) | NOT RUN against a real birth-cert PDF | PASS in unit-tests; PII discipline inherited from passport pipeline; no customer-PDF QA against a real birth cert | yes — falls back if classifier confidence <0.85 or fields missing | 1) No live OCR fixture for a birth certificate.<br>2) No E2E smoke producing a customer PDF.<br>3) No PDF forbidden-phrase scan against a generated birth-cert PDF. | Acquire 1 sanitized birth-certificate fixture (e.g. via `messenginfo.com/upload` test session with a public-domain sample). Run `scripts/phase2-mobile-screenshots.mjs` against it. Capture E2E smoke + forbidden-phrase scan + privacy audit. Save to `artifacts/e2e/birth_cert/`. |
...
- HIGH — no committed live OCR fixture, no E2E smoke, no PDF forbidden-phrase scan against a real birth certificate.
- HIGH — no privacy QA pass against a generated birth-cert customer PDF (only against the passport one).
- MEDIUM — no live verification that the `civil_registry_terms` glossary correctly handles ЗАГС vs РАЦС vs ДРАЦС on a real Soviet-era certificate.
