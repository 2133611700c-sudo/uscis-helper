...
2. **~10 мин — Telegram-бот (если алерты нужны в Telegram).** Через @BotFather
   создать бота → получить `TELEGRAM_BOT_TOKEN`, узнать свой `TELEGRAM_CHAT_ID`,
   прописать в секреты. Без этого алерты идут только на email.
3. **Ongoing — 14-дневный L1 baseline.** `GUARD_BLOCK_METRICS_ENABLED=1` работает
...
4. **Ongoing, 8–16 ч суммарно — GT-масштабирование до N=30/класс.** Собрать
...
5. **Решение — канарейка регистрации паспортных схем.** По шагам
...
6. **Решение — scope US-документов.** Расширять ли переводчик на американские
   документы (и какие). Пока не решено — не строим.
7. **Решение — TPS/Reparole-расширение.** Подключать ли пайплайн к
