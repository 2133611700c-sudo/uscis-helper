...
| **C14** No secrets committed | ✅ PASS | Only `AIzaSy…` strings in history live inside **downloaded uscis.gov HTML** (`docs/uscis/...`) — Google/USCIS public keys, not ours. `.env*`/`.env*.local` gitignored. `service_role` = Postgres role name in SQL, not a JWT. |
