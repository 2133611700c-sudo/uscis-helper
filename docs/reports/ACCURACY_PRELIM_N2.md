...
|---|---|---|---|
| A (all OFF) | 12 | 1 | MISMATCH, MISSED on 2.5-flash / caught on preview primary |
...
- Mode C: `false_negative_review = 0` in all 4 cells; DOB month-mismatch caught; self_consistency =
  agree (2.5-flash this run; mismatch earlier — nondeterministic) / mismatch (preview primary).
- SMART (B vs A): no accuracy gain (consistent with prior N=2).
- 2.5-flash reads a different person (0/5); preview primary 1/5; both unreliable unaided.
...
- model_decision: **NEEDS_MORE_DATA** (preview primary safer on this pair; not enough to switch).
