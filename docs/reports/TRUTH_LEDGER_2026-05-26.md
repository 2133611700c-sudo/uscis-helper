...
Доказательство того, что в проде ZIP/PDF действительно содержат фамилию `Kuropiatnyk` и другие ключевые поля, **есть** (Playwright + `booklet-review-artifacts`).  
...
- **Критерий строгости в тесте:**
  ...
  - убедиться, что в review DOM есть `Kuropiatnyk` / `Trostianets` / `Vinnytsia Oblast` / `Serhiiovych`;
...
- **`booklet-review` (multi-doc)**:
  - ZIP/PDF: **ПОЛУЧЕНЫ**, readback показывает `Kuropiatnyk` в обоих формах.
