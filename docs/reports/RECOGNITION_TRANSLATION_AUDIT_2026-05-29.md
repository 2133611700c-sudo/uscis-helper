...
24. **Нет model-reliability weighting и аудит-следа.** `presence.ts` всегда `candidates:[]`. removed preview primary (20/22) и GV равны по весу.
...
| **Чтение** | значение поля кириллицей; removed preview primary (рукопись), GV (печать), MRZ-парсер (печать) | зона/кроп → cyrillic + confidence | Несколько независимых ридеров; нечитаемо→can_read=false | Частично (1 Gemini + GV-confirm) |
...
| **Gemini (рукопись)** | **ADOPT (уже сделано, верно)** | removed preview primary — ЕДИНСТВЕННЫЙ читающий рукопись (8/9 на 1986). 2.5-pro ФАБРИКУЕТ (1/9), GPT-5.5/4o падают (1/9). Оставить, добавить вес по надёжности, кормить препроцессенными пикселями. |
