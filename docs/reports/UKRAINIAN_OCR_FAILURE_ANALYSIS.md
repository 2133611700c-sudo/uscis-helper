...
certs, identity accuracy ≈ **0–1 of 5** (2.5-flash 0/5 — different person; preview primary 1/5). A dictionary must
...
3. **Ukrainian month misread** — the month read wrong (and inconsistently across runs/models), e.g. GT
   month 06 read as 02 (2.5-flash) / 07 (preview primary).
...
- **Ukrainian internal passport (printed):** **first owner-GT datapoint** through the live door @ preview primary —
...
- **Hard-case Ukrainian / Soviet / handwritten birth certs:** **critical failure** — 1/4 identity fields
  correct even on preview primary (only family_name; given/patronymic/DOB wrong, re-confirmed vs owner GT this
...
- hard-case model: **UNRESOLVED_BLOCKER** (neither 2.5-flash nor preview primary reads UA hard-case reliably; a
