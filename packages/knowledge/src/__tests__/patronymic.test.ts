 ...
 * (test-fixtures/real-docs/): birth cert (Kuropiatnyk), marriage cert
...
check('Сергій → M Сергійович', generatePatronymic('Сергій', 'M').value, 'Сергійович')
check('Сергій → F Сергіївна',  generatePatronymic('Сергій', 'F').value, 'Сергіївна')
...
check('Сергійович valid (M)',  isValidPatronymic('Сергійович', 'M'), true)
...
check('digits rejected',          isValidPatronymic('Серг1йович', 'M'), false)
check('empty rejected',           isValidPatronymic('', 'M'), false)

console.log('reconcilePatronymic — Chief Engineer entry point')
check('valid read kept, no review', reconcilePatronymic('Сергійович', 'Сергій', 'M'),
  { value: 'Сергійович', source: 'read_valid', review_required: false, reason: 'read is complete and well-formed' })
check('fragment "ович" → reconstructed from given name', reconcilePatronymic('ович', 'Сергій', 'M').value, 'Сергійович')
check('fragment reconstructed flags review', reconcilePatronymic('ович', 'Сергій', 'M').review_required, true)
