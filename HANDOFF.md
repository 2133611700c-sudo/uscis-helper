...
- **Verified:** `208215f` really closed the user-facing evidence payoff, but it still had one honesty hole: when `EvidenceRegion.page` referenced a page the client did not have, the wizard silently fell back to `previewUrls[0]`. That could display a crop from the wrong page. Fixed by introducing `resolveEvidenceImageUrl()` and rendering **no crop** unless the exact preview page exists.
- **Verified:** the temporary GPT route (`READER_PROVIDER=openai`) was still too broad. It routed handwritten/certificate families through OpenAI even though the documented model evidence says GPT is unsafe there. Fixed at the selector seam: handwritten/certificate doc families now stay on the default reader path; GPT override remains available for printed docs.
- **Files changed:** `apps/web/src/components/services/translation/TranslateWizard.tsx`, `apps/web/src/components/services/translation/fieldEvidenceCrop.ts`, `apps/web/src/components/services/translation/__tests__/fieldEvidenceCrop.test.ts`, `apps/web/src/lib/docintel/documentFieldReader.ts`, `apps/web/src/lib/docintel/__tests__/readerProviderSelect.test.ts`, `CHANGELOG.md`, `STATUS.md`, `HANDOFF.md`.
- **Evidence:** full web suite rerun after the fixes = **376 files / 5083 tests passed / 26 skipped / 0 failed**; `CI=true pnpm --filter web run typecheck` PASS; `CI=true pnpm --filter web run build` PASS with the same pre-existing Next/Sentry/libheif + TranslateWizard lint warnings only.
- **Still NOT done / not to overclaim:** no push, no CI, no live prod/staging proof, no `ONE_BRAIN_RECOGNIZE_ENABLED=1` rollout proof, no exact-bbox Vision/DocAI path, no GT benchmark rerun.
- **Next exact task:** either push this branch for CI proof, or continue the remaining local-safe hardening on the same seam (for example, document-family/PII policy tightening around the temporary OpenAI route). Do NOT claim release/runtime completion from this branch alone.

## 2026-06-28 | Route cutover COMPLETE — ratchet 0 (was 13) — APPLICATION CODE COMPLETE for routes
- **ALL 13 active translation routes migrated to `getRepositories()`:** upload, ocr-from-storage, extract, extraction-status, manual-review-status, review-state, confirm-field, correct-field, certify, delete, process, render, generate-pdf. Zero direct Supabase imports in the route layer (ratchet `noDirectSupabaseInDomain.test.ts` now asserts `toEqual([])` + `KNOWN_COUPLED_ROUTES.size===0`).
- **Repository bundle (final surface):** documents (getSession/getDocument/getLatestDocument/createDocument/markUploaded/markExtracted/updateSessionStatus), review (listFields/upsertFields/getField), confirmation (confirmField/correctField/recordUserCorrection), translation, pdfArtifacts, audit (append/list), manualReview (getLatestTicket/getCase/deleteCase), extractionRuns (getRun/countFields/createRun/updateRun), storage (remove/createSignedUrl/upload/download), certification (save/getCertificationRecord), orders (getOrder/updateOrder/appendEvent), finalRenders (save/getFinalRender), certificationAudit (appendOrderRow/appendCertificationAudit). Plus `repositoryInsertableClient()` adapter (for the client-agnostic persistCertification lib).
- **In-memory** is the default driver (process singleton); the Supabase adapter is a fail-closed STUB (every method throws SupabaseNotConnectedError) until the owner wires it. NO Supabase connection, NO migrations run, NO secrets, NO real DB writes.
- **Evidence:** full suite 4944 passed / 26 skipped / 0 failed; tsc 0; PII clean. All flags default OFF.
- **NEXT (owner-gated, Track B):** run the Supabase connection sequence in `docs/architecture/SUPABASE_CONNECTION_PLAN.md` (link staging → db push migrations → seed synthetic → implement the real Supabase adapter against the SAME contract suite → set `REPOSITORY_DRIVER=supabase` → run `repositoryContract.test.ts` against the adapter → DB-backed browser E2E). Do NOT connect Supabase / enable production flags without owner approval. Status: **APPLICATION CODE COMPLETE (routes) — READY FOR DATABASE-BACKED STAGING VALIDATION**; NOT production-ready (no DB-backed E2E + owner sign-off).

## 2026-06-28 | Route cutover progress — ratchet 1 (was 13)
- **Migrated to `getRepositories()` so far:** confirm-field, extract, extraction-status, manual-review-status, correct-field, delete, certify, review-state, upload, process, ocr-from-storage, **render (this session)**. Each route imports 0 Supabase clients; in-memory default; Supabase adapter fail-closed.
- **Remaining coupled route (1) — ratchet `KNOWN_COUPLED_ROUTES`:** generate-pdf — THE LAST ONE. The heaviest: Stripe payment verify (with expectedService) + pdf-lib render + Resend email-with-attachment + canonical continuity + final-PDF gate + 11-gate validation + certification audit (`translation_certification_audit`) + `orders` insert (via `persistCertification`). Only the Supabase persistence parts move; Stripe/Resend/pdf-lib/canonical-hash/documentSafety guards stay direct deps.
- **Repos available now:** documents (getSession/getDocument/getLatestDocument/createDocument/markUploaded/markExtracted/updateSessionStatus), review (listFields/upsertFields/getField), confirmation (confirmField/correctField/recordUserCorrection), translation, pdfArtifacts, audit (append/list), manualReview (getLatestTicket/getCase/deleteCase), extractionRuns (getRun/countFields/createRun/updateRun), storage (remove/createSignedUrl/upload/download), certification (save/getCertificationRecord), orders (getOrder/updateOrder/appendEvent), finalRenders (save/getFinalRender).
- **Repos still needed for generate-pdf:** likely a certification-audit persistence method (reuse OrderRepository for the `orders` insert; `persistCertification` in `@/lib/...` may itself wrap supabase as a lib — CHECK whether it's a direct supabase import in the ROUTE or a lib import; only direct route imports are flagged by the ratchet). Read the route first (`apps/web/src/app/api/translation/generate-pdf/route.ts`, ~500 lines) to inventory exact `supabase.` usages vs lib wrappers.
- **Next exact task:** cutover #13 = generate-pdf → `getRepositories()`. Inventory every `supabase.from/.storage` call; map session/fields/cert reads to documents/review/certification repos; map `orders` insert + `translation_certification_audit` insert to repos (extend OrderRepository or add a small CertificationAuditRepository); keep Stripe/Resend/pdf-lib/canonical/guards direct. Add an in-memory test (mock Stripe/Resend/pdf). Remove generate-pdf from the ratchet, then **flip the final ratchet assertion `documents remaining backlog count` to `toBe(0)`** → APPLICATION CODE COMPLETE for routes.
...
- **Domain/canonical-core/review/PDF library layers are Supabase-client-free** and LOCKED by an import-boundary invariant + ratchet (`noDirectSupabaseInDomain.test.ts`): lib/contracts, lib/repositories (runtime), lib/translation/pdf, lib/canonical/core import 0 Supabase clients; a NEW coupled route fails the test.
- **OUTSTANDING — 13 active translation ROUTE handlers still import Supabase directly** (the ratchet backlog). Full cutover to `getRepositories()` is ALL-OR-NOTHING (shared session lifecycle) across ~11 tables incl. payment/cert/storage (`translation_sessions, extracted_fields, audit_logs, certification_records, final_renders, translation_documents, extraction_runs, manual_review_queue, user_corrections, translation_events, translation_orders`). Not done as a big-bang to avoid breaking checkout/cert/delivery + the regression suite.
- **To complete (dedicated coordinated effort):** expand `RepositoryBundle` to model the remaining concepts (certification, final renders, documents, extraction runs, manual-review queue, corrections, orders/events); port each route to `getRepositories()`; rewrite each route's Supabase-mocking test to in-memory; add route-level integration tests (create/load/extraction/review/correction/confirmation/PDF-artifact/retry-idempotency/not-found/unauthorized/failure). Then flip the ratchet's last assertion to `toBe(0)`. Only THEN: **APPLICATION CODE COMPLETE — READY FOR SUPABASE ADAPTER CONNECTION AND DATABASE CUTOVER VALIDATION**.

## 2026-06-28 | Repository abstraction + Supabase prep — CODE COMPLETE (library layer) — READY FOR DATABASE-BACKED STAGING VALIDATION
...
- Military GT cleaned: single `handwritten:true`, removed stale model-draft provenance.
...
- The weak part is also real: `6919dbe` is **docs-only** and there is still no committed, reproducible cross-hand harness/report for the military result. The military GT metadata remains internally inconsistent (`handwritten:false` + `handwritten_actual:true`, stale model-draft wording), so this corpus contract still needs cleanup before anyone treats it as finalized evidence.
...
- Removed literal forbidden-family Gemini version references from active runtime code, active scripts, and authoritative architecture docs.
- `normalizeGeminiModel()` now accepts only the sanctioned chain from `modelMatrix.ts`; stale env ids fall back automatically.
- Current authoritative runtime truth: `gemini-2.5-pro` primary, `gemini-3.5-flash` and `gemini-2.5-flash` fallbacks, handwriting handled outside LLM acceptance.
- Remaining old preview-primary wording below is historical context only, not current law.
- CI `gemini-quota-diag.yml` repointed off the removed preview → `gemini-2.5-pro`; tracked tree now has 0 literal forbidden-family ids.
- **PII guard hardened:** fixed a tokenizer gap (trailing quote/apostrophe folded into the token) that let denylisted PII pass inside string literals / filename suffixes. Fix surfaced + cleaned previously-missed owner-surname occurrences in HTR tests and docs (now fictional). Guard + `piiGuard.test.ts` green; `tsc` has 0 non-`openai` errors.
- **OPEN (needs decision):** (1) owner surname still exists in already-pushed history (`197f1c6`) — working-tree is clean but a full purge needs a history rewrite (private repo → likely accept). (2) OpenAI key rotation pending (was pasted in chat). (3) `openai` npm package not installable offline → full typecheck/vitest blocked by dependency install, not by code.

## 2026-06-25 | Deterministic handwritten birth-cert crop regression fixed — template path now matches the verified patronymic crop
- **What changed:** narrowed the runtime handwritten template for `ua_birth_certificate.patronymic` in `apps/web/src/lib/docintel/ensemble/handwrittenFieldRoute.ts` from the older wide box to the tuned box proven in the frozen bake-off manifest. Added a regression assertion in `apps/web/src/lib/docintel/ensemble/__tests__/handwrittenFieldRoute.test.ts`.
- **Root cause:** the repo had already proven a better patronymic crop in `qa-private/htr-poc/bakeoff_manifest.json` (`[1090,905,1850,1120]`), but the live deterministic template still used the older wider crop. That overlap with the given-name region caused the operational HTR path to emit `гей Богданович` instead of `Богданович`.
- **Verified after fix (real sidecar, real doc, no mocks):**
  - fixture: `test-fixtures/real-docs/birth_cert_handwritten_01.jpg`
  - path: `readHandwrittenRoute(...)` with `HTR_SIDECAR_URL=http://127.0.0.1:8077`
  - output now = `Соловьяк / Андрей / Богданович`, all `review_required=true`
  - A/B on the same file through `readHandwrittenFieldsViaSidecar`: old box → `гей Богданович`; new box → `Богданович`
- **Tests/gates:** targeted vitest suite PASS (11/11) and `pnpm exec tsc --noEmit` PASS in `apps/web`.
- **Exact next step:** prove the same fixed handwritten path through the actual browser/upload/review flow, not only module-level reads. The remaining product question is no longer “which model?”, but whether the real upload path preserves the same bytes/orientation/crop geometry into review UI.

## 2026-06-24 | OpenAI/GPT handwritten-cert audit completed — no direct VLM winner
- Owner asked for a fresh independent comparison of current GPT-family candidates after auditing the real pixel path first. Done on the real handwritten birth certificate fixture, with live preprocess + content-orientation verified. `removed preview primary` explicitly excluded.
- Results:
...
- `documentFieldReader.ts` `runHtrFieldStage` (normal path + the `!read.ok` early-return → `htr_only:*`) — for `isHandwrittenFamily(docTypeId)` + sidecar configured + original buffer, the HTR read is the AUTHORITATIVE `raw_cyrillic` per handwritten name field, ALWAYS review-gated, and reads INDEPENDENTLY of the LLM (runs even when the LLM read fails). **FAIL-CLOSED:** HTR unavailable / empty / low-confidence on a critical handwritten field → `value=''` (→ C3 `final_value=null`) + `review_required` → autofill blocked; the weaker LLM cursive read is NEVER trusted. (Integration-tested: `htrOnlyEarlyReturn.test.ts` proves `htr_only` on LLM-503 + fail-closed when sidecar down.)
...
- **STILL TRUE (honest, NOT disputed):** NOT production-grade — proof battery on the owner-verified gold set remains small, `raxtemur` cannot abstain, and broad runtime E2E is still unproven. The old "1939 FAILS" claim was a localization mistake and is superseded by the later frozen bake-off on corrected crops; that improves the anecdote but does not prove generality. Reports: `docs/research/HTR_LOCAL_API_AND_BATTERY.md`, `docs/research/HTR_STABLE_BENCHMARK.md`.
- **REMAINING WORK is NOT just owner resources:** engineering still owes (1) automatic field localization robust enough for runtime, not only hand-frozen boxes; (2) staging/site E2E proof through the real upload path; (3) failure-mode proof for sidecar unavailable / malformed / low-confidence reads; (4) broader owner-verified held-out GT before any production-grade claim.


## 2026-06-24 | ROOT-CAUSE REVERSAL — handwritten UA/RU Cyrillic IS readable key-free (raxtemur), blocker was OUR pipeline
**Done:** 4 parallel root-cause agents + independent reproduction OVERTURNED the prior "no key-free model reads the surname" finding. **raxtemur/trocr-base-ru (Apache, local, no keys)** reads the child name EXACTLY given a NATIVE-resolution crop (from 4128×3096) + contrast-stretch (NO downscale, NO binarize): surname CER 0.000 (3/3), given 0.000, patronymic 0.333 (matches). Critical correction: blank-control was NOT clean — raxtemur emits non-empty text on blank input, so review remains mandatory. Root bugs: (1) crop downscaled to h128 destroyed signal [binding]; (2) scorer Latin-vs-Cyrillic channel bug; (3) label/stamp contamination. Data was NOT the constraint; synthetic fine-tune abandoned. Wrote ADR-026 (production pipeline rule), corrected MODEL_INVENTORY + HTR_ZEROSHOT_POC. Verified by `qa-private/htr-poc/verify_root.py`.
...
**EXACT NEXT TASK:** (1) native-res PER-FIELD reader as the PRIMARY path (today: full-downscaled-page→Gemini + tileRegionRead recovery) + automatic field-region localization (proof used hand-found boxes) — needs the raxtemur sidecar HOST decision (Python/torch, not Vercel); (2) re-test Gemini/GPT on native-res crops (their "fabrication" may be a low-res artifact); (3) owner: fill UA GT in qa-private to measure UA; (4) broader-N validation before relaxing the human-review gate. Scope = modern UA & RU Cyrillic.

## (superseded same day) HTR zero-shot POC — open Cyrillic models on the real cert (key-free baseline)
...
**Bake-off COMPLETE (4 local candidates):** added PyLaia CRNN-CTC `crnn-ctc-ukrainian`+`-church-slavonic` (Apache, via polyscriptor MIT loader) and stamp-suppression. UA-CRNN surname CER 0.90/conf 0.33, given CER 1.0 (worse than TrOCR); CS transfer-gap confirmed. PyLaia LOW conf on errors = usable reviewer flag. Stamp = ~5% of crop, removal moved raxtemur surname 1.10→0.70 (partial) → blocker is cursive, not stamp. **NO key-free model reads the cursive surname zero-shot — across transformer + CRNN architectures.**
...
- Renamed gitignored fixtures `*_soloviak`→`*_01` (disk + all committed refs) and moved the committed real-reads dump → gitignored `qa-private/reports/` (it was committed PII). `gt-pipeline-bench.mjs` default path updated + skips gracefully when absent.
- NEW hash-based PII guard `scripts/check-no-pii.mjs` + `scripts/pii-denylist.sha256.json` (HASHES only) + gitignored generator `qa-private/tools/gen-pii-denylist.mjs`. `guards.yml` rewired to it (secret-free); deleted leaky `.pii-patterns`/`.pii-secret-setup.txt`.
- Scope kept tight (no false positives): common given names + the dual-use PUBLIC town Тростянець are intentionally NOT scrubbed/hashed.
- **Evidence:** PII guard GREEN 0/1781 tracked files · apps/web 4656 pass · knowledge suite pass · tsc 0 · `gt-pipeline-bench --dry` green ($0).
- **Live Gemini (Step 6, ~$0.17 total):** billing UNBLOCKED (the "429-capped" memory was STALE). Pipeline + Tier-2 run live; one good window = 60% on passport with orient+tile firing.
- **ROOT CAUSE of the качели found + fixed:** `removed preview primary` is a PREVIEW endpoint (no GA `removed preview primary` exists) → inherent 503/deadlines, unfixable from console. The retry path was DEAD (`setTimeout(1500);continue` exited the `a<1` loop → primary never retried → instant flash fallback). Replaced with real exponential-backoff+jitter retry on the primary (`geminiVisionProvider.ts`); knobs `GEMINI_PRIMARY_RETRY_MAX`/`_RETRY_BASE_MS`/`_RETRY_CAP_MS`. ADR-018-safe. Live probe confirms the primary now retries + succeeds on calls it previously abandoned; tsc 0, 4658 tests pass.

**EXACT NEXT TASK:**
1. Re-run `STAB_RUNS=3 ORIENT_VOTE_RUNS=2 TILE_VOTE_RUNS=2 npx tsx --tsconfig tsconfig.json apps/web/scripts/stability-audit.mjs` when Google's preview capacity recovers — the backoff fix should now land primary reads on the heavy full-page request (it 503'd through in the current degraded window despite retries).
2. OWNER DECISION (amends ADR-018): add GA `gemini-2.5-pro` to the fallback chain above flash (modelMatrix.ts FALLBACK_MODELS + test) — better degraded quality than flash, still force-reviewed. Requires verifying it's callable + has NO different-person bug (the reason 2.5-flash is disqualified for certs).
3. Optionally `git commit` (guard green, suites green).
...
2. RESOLVE TWO UNKNOWNS the baseline exposed: (a) is `sex` a real pipeline MISS or just absent from the R0 dry-reads fixture? (b) GT VALIDITY — `birth_cert_soviet` GT expects UA `Андрій` but the doc is RU `Андрей`; a source-faithful certified translation transliterates the Russian as written, so the 20% may be artificially low. Owner reconciliation on RU-script name policy.
...
4. Account-side (owner): raise `removed preview primary` RPM to kill the flash-fallback→force-review cascade (the real R12 fix).
...
2. STILL OPEN from prior session (unchanged): primary removed preview primary RPM throttling on heavy vision → fallback reads force-reviewed (account-side, not code).
...
- Honest caveats recorded in-file: detectNameScript='unknown' for shared-letter names (correct conservative); transliterateKMU55('Андрей')=Serhei (RU uses transliterateRussian→Andrey); MRZ all-caps name-cased to title by brain (asserted case-insensitively).
...
- CALL-SITE INVENTORY (file:line, provider, model, default-ON, est cost — public list prices in OCR_COST_TABLE_USD_MICROS, sources cited in code):
  ...
  3. src/lib/docintel/providers/geminiVisionProvider.ts:~89 callGemini — Gemini generateContent (removed preview primary→3.5/2.5-flash fallback) — the docintel readDocument reader for EAD/Reparole/Translation — ~$0.002/call (2000).
...
1. **ADR-018** (`docs/adr/ADR-018-model-matrix.md`): permanent model-to-operation matrix. Verified against code:
   - removed preview primary = THE reader (prod env clean, smoke PASS) ✓
...
6. **PASS_PROD_MODEL_SMOKE:** prod on `removed preview primary` confirmed live.
...
# HANDOFF (2026-06-10 — PASS_PROD_MODEL_SMOKE: prod on removed preview primary, Phase 3 UNBLOCKED)

## What was done this session
1. **Prod GEMINI_MODEL flip:** verified dirty value `"gemini-2.5-flash\n"` (literal embedded `\n`, made flash the effective prod model). Removed and replaced with clean `removed preview primary` via `vercel env rm` + `printf | vercel env add`.
...
4. **Live smoke:** `POST /api/translation/vision-extract` (1×1 PNG, `docTypeId=us_i94`, no PII) → response `model: removed preview primary` confirmed, 4554ms, no 5xx, no fallback.
...
cert + removed preview primary + flags ON) → 5/5 identity forced review + reasons + values unchanged + self_consistency
...
<!-- 2026-06-29: Critical one-brain audit closure on local branch feat/one-brain-reader-result (HEAD 1a3267a, unpushed). VERIFIED by direct reruns: full `pnpm --filter web test` now green (5074 passed / 26 skipped), `typecheck` PASS, `build` PASS with warnings. Root cause of earlier "not all green" finding was suite-level timeout instability, not assertion failure: piiGuard.test.ts and knowledgeSafetyNet.test.ts passed alone but timed out under full load at 30s; fixed by per-file vi.setConfig({ testTimeout: 120_000 }). Also corrected branch truth drift: docs/ocr/ONE_BRAIN_CONVERGENCE.md no longer claims bbox/evidence is unwired, because Translation backend already attaches template evidence behind ONE_BRAIN_EVIDENCE_ENABLED; EAD/ReParole route comments and `_flag` observability now name the real runtime gate ONE_BRAIN_RECOGNIZE_ENABLED. Remaining honest gaps: branch still local-only (NO_UPSTREAM), no GitHub CI proof for this head, no live/prod proof, review UI still does not render EvidenceRegion crops, and GPT route / owner-resource claims remain UNVERIFIED in this audit unless rerun live. Next exact action: push this branch to a remote branch and run GitHub CI before making any stronger release claim. -->
<!-- 2026-07-04: transliteration audit handoff — deterministic KMU-55 / Russian transliteration works on direct-import spot checks; mixed-script guard works on mixed tokens; prescribed pnpm/vitest execution is blocked in this workspace by node_modules relink EPERM / missing local binaries. Next exact action: fix the workspace test runner, then rerun the transliteration suites and keep the report verdict at TRANSLITERATION_SPOT_PASS / RUNNER_BLOCKED unless the runner becomes fully available. -->
...
<!-- 2026-07-05: orientation handoff follow-up — handwritten doc classes now run the 180° confirm by default even when `ORIENT_180_CHECK` is unset. Live probe on real handwritten fixtures corrected the prior 180° miscorrections on `birth_cert_handwritten_01` and `military_id_p1_01`; the printed/sparse fixtures already stayed stable. Verified by targeted vitest, `tsc --noEmit`, and `node scripts/check-no-pii.mjs`. -->
<!-- 2026-07-06: one-brain probe handoff — added `apps/web/scripts/one-brain-orient-read.mts` and `apps/web/src/lib/docintel/oneBrainTool.ts`. Live probes on `birth_cert_handwritten_01.jpg` and `military_id_p1_01.jpg` both got upright orientation from content_orient cw=90. Gemini was quota-blocked (HTTP 429), so the script now supports an explicit `--provider openai` fallback; that fallback read both docs successfully and returned handwritten Cyrillic fields with review_required=true. Verified by targeted vitest, `tsc --noEmit`, and `node scripts/check-no-pii.mjs`. -->
<!-- 2026-07-06: alias follow-up — the CLI now normalizes common file-name aliases such as `internal_passport_01` to `ua_internal_passport_booklet`. The fixture is accepted, but the openai fallback still returns `ok` with 0 handwritten fields, so this document class is still a genuine gap, not a solved reader case. -->
<!-- 2026-07-06: crop-route follow-up — `HANDWRITING_CROP_LLM=openai` is now supported for the handwritten crop route. The route is still limited to handwritten families; `ua_internal_passport_booklet` is a printed doc in the model matrix, so its 0 handwritten fields are expected. -->
