...
- Ground truth for `qa-private/ground-truth/passport_international_kuropiatnyk.json` still has `ground_truth_status: MISSING` — no real document fields to verify against.
- PR not yet merged (branch: `feat/mrz-passport-authority`).

**Next exact task:**
1. Owner fills ground truth in `qa-private/ground-truth/passport_international_kuropiatnyk.json`.
