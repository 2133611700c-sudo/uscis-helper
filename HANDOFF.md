...
- Pre-push safety: clean tracked tree; `qa-private/`+`reports/` ignored; `docs/reports/` not ignored;
  0 tracked private files; **0 actual credentials** in the diff. PII: `PASSPORT-REDACTED`/surname/DOB are
...
- Ran live engine baseline:
  ...
  - fail: passport fixture in `engine/presence.ts` produced empty Latin fields after OCR-confirm guard; assertion on `REDACTED` failed
...
- Verified hard blocker:
  - `test-fixtures/real-docs/` currently has only one booklet fixture: `internal_passport_redacted.jpg`
