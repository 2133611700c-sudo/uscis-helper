...
NOT DONE / left for owner: do NOT merge yet; do NOT implement cache substitution or budget enforcement here (7-B/7-C, staging-gated). The summary/event log lines are now visible in prod logs once deployed — owner can later aggregate ocr_upload_cost_summary to size a data-driven cap.
NEXT (owner): review PR → after a deploy, collect ocr_provider_call/ocr_upload_cost_summary from logs to (a) confirm real per-upload call counts/cost and (b) compute would-be cache-hit rate from cache_key_sha repeats → then 7-B cache + 7-C cap (staging-gated).
