# HANDOFF (2026-06-14 — P1 Translation server PII ledger WIRED into LIVE TranslateWizard behind flag, OFF-path parity)
DONE: Branch fix/p1-translation-server-ledger-wiring off main (4cfca97). Mirrors TPS (#139) + Re-Parole, adapted for Translation's sessionStorage + Stripe round-trip + OPERATOR_FLOW. Target apps/web/src/components/services/translation/TranslateWizard.tsx (persisted DraftState {screen, selectedDocType, extractedFields[{field,value,raw_cyrillic,review_required}], canonicalDocumentId, savedAt} to sessionStorage DRAFT_KEY 'tw:v2:draft'; raw_cyrillic = documented PII exception). WIRED @/lib/v1/wizardLedgerClient (REUSED unmodified) behind flag NEXT_PUBLIC_SERVER_LEDGER_ENABLED (default OFF):
- SAVE (saveDraft useCallback, now async): build one DraftState; ON → wipe legacy sessionStorage DRAFT_KEY (defensive) + `await saveDraftToServer('translation', draft)`; OFF → `sessionStorage.setItem(DRAFT_KEY, JSON.stringify(draft))`. handlePayment + owner path `await saveDraft()` so the opaque token cookie is set BEFORE the Stripe redirect (the cookie is what survives the round-trip).
- HYDRATE (?paid=1 mount restore): ON → wipe legacy DRAFT_KEY + `loadDraftFromServer<DraftState>()` → shared applyDraft() (TTL via isDraftExpired, screen guard, restore docType/fields/canonicalId); OFF → `sessionStorage.getItem(DRAFT_KEY)` + TTL precheck → applyDraft(). Operator-flow + ticket-creation reads use a shared `readPersistedDraft()` (ON loadDraftFromServer, OFF sessionStorage) so the post-payment submit-order/manual-review get the draft from the right source.
- CLEAR: submit-order success (operator handoff) via shared `clearPersistedDraft()` (ON clearServerDraft + remove 'tw:cs'; OFF sessionStorage.removeItem DRAFT_KEY+'tw:cs'); resetAll/start-over → ON clearServerDraft, OFF sessionStorage removes. Legacy self-serve PDF path does NOT clear (unchanged — only submit-order deletes the draft per owner spec).
- TTL: server-side 24h (route/store) → null on expiry; client applyDraft also rejects expired via isDraftExpired. Cancel/error keeps draft until TTL (no clear on payment cancel).
- canonical_document_id: stays in DraftState (saveDraft) and is restored on hydrate; flows to generate-pdf (X canonical_document_id) and submit-order in BOTH states.
OFF-PATH PARITY PROOF: every ledger primitive inside `if (isLedgerClientEnabled()){…}else{<byte-identical pre-existing sessionStorage>}`; flag default OFF (NEXT_PUBLIC_SERVER_LEDGER_ENABLED not set in any .env). Flag OFF → ledger never called, sessionStorage shape/keys unchanged → OFF deploy is a no-op. Static tests assert each branch + readPersistedDraft source-switch + `await saveDraft()` against TranslateWizard source.
ON-PATH BROWSER-PII=0 PROOF (incl. raw_cyrillic): integration test routes the real wizardLedgerClient through the real POST/GET/DELETE handlers (in-memory Supabase double) carrying only the opaque cookie; asserts the browser cookie jar holds ONLY wizard_draft_token (64-hex) and contains NONE of value/raw_cyrillic (Шевченко/Тарас/Київ/SHEVCHENKO/DOB), server row ciphertext-only. canonical survives a simulated ?paid=1 round-trip (fresh fetch shim seeded only with the cookie → GET restores canonicalDocumentId + 4 fields). Real-browser Playwright BLOCKED_EXTERNAL (no local Postgres/Docker; needs staging w/ flag ON + WIZARD_DRAFT_ENC_KEY + wizard_drafts table) — proven at the integration layer (no fake green).
TESTS: NEW src/components/services/translation/__tests__/translateWizardServerLedger.itest.test.ts (15 cases). Updated ownerMode.test.ts static regex for `await saveDraft()`. 3868 pass / 24 skip (no decrease; +15). GATES: tsc 0 real; build OK; content-guard 0; STATUS single # H1.
NOT DONE / left for owner: do NOT merge; do NOT change prod flag (still OFF); enforce NOT touched. TPS + Re-Parole source UNTOUCHED. Real-browser Playwright BLOCKED_EXTERNAL (staging deploy with both flags ON + key + wizard_drafts table required).
NEXT (owner): review PR → deploy to staging with both flags ON to run the live-browser invariant → decide whether to flip the prod flag (separate deliberate action).
