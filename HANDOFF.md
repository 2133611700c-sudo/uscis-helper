# HANDOFF (2026-06-09 — REBUILD START: ONE Gemini brain, ADR-017, Phase 1.1 done)
...
**Phase 1.1 landed (CODE, not docs):** `apps/web/src/lib/canonical/core/knowledgeNormalize.ts` — pure deterministic "dictionary in the brain" (KMU-55 / gazetteer / patronymic / oblast→nominative / authority on the FINAL value; Latin/MRZ preserved; never-silent fuzzy→review). 8 tests green; tsc 0. Pure/unwired = byte-identical. Branch `feat/one-brain-gemini-core` off origin/main `03eb30f`. No prod/keys/PII.
**Next (Phase 1.2–1.4):** wire `normalizeCanonicalValue` into `canonical/core/arbitration.ts` `normalizedValue` behind `KNOWLEDGE_BRAIN_ENABLED` (default OFF → byte-identical); thread doc-class/sex/given-name from adapters; real-fixture proof. Then Phase 2 = consolidate all products onto one Core path + remove GPT + retire legacy `/api/ocr/extract` fork. Full plan: docs/reports/ONE_BRAIN_GEMINI_BUILD_PLAN.md. ADR: docs/adr/ADR-017-one-gemini-brain-not-consensus.md. Keys/prod = owner-managed (agent builds to preview-ready).
