...
**NEXT TASK: Phase 2.0b** — Remove deprecated `gemini-2.0-flash` (HTTP 404) from `geminiVisionProvider` fallback chain. Small targeted fix: find the fallback model array in `apps/web/src/lib/docintel/geminiVisionProvider.ts`, remove the 404 entry, verify no other callers use it, tsc + suite pass.

After 2.0b → **Phase 2.1** (Translation → Core default, preview-prove + PDF, one product at a time).
