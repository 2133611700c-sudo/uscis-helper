...
---
# HANDOFF (2026-06-13 — Phase 2 Agent 3: internal harness — payment boundary + operator E2E + chaos, NO external Stripe)

DONE: Proved the ENTIRE Phase 2 pipeline EXCEPT the hosted Stripe Checkout positive E2E (owner-deferred to final acceptance), using in-process mocks/synthetic fixtures. (PAYMENT BOUNDARY) Identified the EXACT verification boundary: the V2 operator pipeline's payment AUTHORITY is the INLINE `verifyStripeSessionPaid()` in `submit-order` (the Stripe checkout token is the capability) — NOT the webhook. The Stripe webhook (`api/stripe/webhook/route.ts`) is a side-channel that only touches the LEGACY `translation_orders` table (status `signed`→`emailed` by verified email), never creates V2 orders. Both tested. (NEW SUITES) (1) `api/stripe/webhook/__tests__/webhookPaymentBoundary.test.ts` (8) — behavioral, mocks the Stripe SDK `constructEvent` boundary in-test: missing-signature→400, missing-secret→400 fail-closed, invalid-signature(SDK throws)→400 no DB writes, wrong event type→no transition, payment_failed→never marks paid, verified checkout.session.completed→audit + legacy update gated on status='signed'+email match, no-email→no update, wrong product(tps)→no translation/wizard update. (2) `lib/translation/orders/__tests__/operatorPipelineBehavioral.test.ts` (29) — drives the REAL `orders/index.ts` + `v2Actions` + delivery worker against a faithful in-process fake of the Phase 2 Supabase surface (`fakeOrdersDb.ts`, mirrors the SQL RPCs/triggers exactly): order created exactly once + idempotent replay (recipient never overwritten by replay), recipient from server-verified value only, canonical binding immutable, version-increment + audit-event-append, actor-required; artifact+outbox ATOMIC, INV-11 C3-null omitted, duplicate idempotency_key→no 2nd outbox; operator authz (401 no-cookie / 403 unconfigured / 409 stale version), invalid transition blocked, override provenance=operator_override + base preserved + empty=explicit-reject-null, changeRecipient separate audited (confirm+reason, hashes only); delivery worker exactly-once + hash-verify-before-send + NO re-render + idempotency-key flow, second-run no duplicate, two concurrent claims→one wins, tampered bytes→hash-mismatch→retry not deliver, transport-fail→retryable, crash-after-send→no duplicate, missing-recipient→permanent fail; CHAOS: DB-fail-between-artifact-and-outbox→full rollback (no orphan outbox, order stays approved_for_render), storage-fail→no artifact/outbox, operator-double-click→1 event, retryDelivery re-arms outbox (no re-render) + re-delivers once, worker bad-bearer→401 nothing drained. (3) `phase2OrdersInvariants.live.test.ts` (8, SELF-SKIPS unless `RUN_DB_INVARIANTS=1`+service-role env) — same invariants against REAL Supabase with `PHASE2_TEST_` sentinels, cleaned via guarded `phase2_admin_cleanup`. (SEAM) Made the email transport injectable: `lib/email/resend.ts` now has `EmailTransport` + `setEmailTransportForTesting`/`resetEmailTransport`; prod default UNCHANGED (real Resend `realResendSend`); tests inject a capturing transport (no real send, PII-free capture). No product code calls the test seam.

NOT DONE (owner-deferred, binding): the hosted Stripe Checkout POSITIVE E2E (real `cs_*` session, real card) — deferred to final acceptance. Did NOT request Stripe keys, did NOT mark-as-paid in prod, did NOT add any payment-bypass route. Prod stays SHADOW; PR #119 stays DRAFT; no merge.

NEXT TASK (final acceptance): run the hosted Stripe positive E2E with test-mode keys against a preview, then the `phase2OrdersInvariants.live.test.ts` with `RUN_DB_INVARIANTS=1` against the real DB. RECOMMENDATION (in evidence): promote a Stripe WEBHOOK (`checkout.session.completed`, signature-verified) to V2 authority — today V2 order creation rides the inline session-verify on the client's submit-order POST; a client that never returns from Checkout leaves no V2 order. A webhook is the durable "payment happened" signal.

EVIDENCE: input_sha=44ec7b8fcaa61ac621dbb1f2a8e14b64ea8eab44. tsc 0 errors. Full vitest 3764 pass / 46 skip / 0 fail (228 files) — +65 new pass (8 webhook + 29 behavioral + 24 email-seam-regression-clean + 4 unchanged delivery-worker) + 8 new skipped (live, gated). New-suite focused run 65 pass / 8 skip. output_sha(operatorPipelineBehavioral.test.ts)=d5b9313b32c1e9a3b739b52b44de34c96f7975177a2a8e8d1fe578ff827e7813. No external Stripe, no live DB touched (live suite self-skipped), no prod env/flag change, no merge.

---
# HANDOFF (2026-06-13 — Phase 2 Agent 4: observability/operations + independent audit)

DONE: Built the observability/operations layer on Agent 1-3's Phase 2 foundation (branch `architecture/translation-operator-canonical-v2`, base 44ec7b8). ADDITIVE, no behavior change, prod stays SHADOW, PR not merged. (A) `apps/web/src/lib/translation/observability/events.ts` — typed PII-safe event emitter, 25 codes (orders/operator/artifact/delivery/payment_boundary). PII is unrepresentable by type; `assertPiiSafe()` rejects forbidden/unknown keys, email-like values, over-long text, even strings hidden in `field_keys[]`. `emitEvent()` never throws into caller; silent unless `PHASE2_EVENTS_ENABLED=1`. Wired into submit-order, v2Actions (override/artifact/auth/stale-version), delivery worker, stripe webhook. (B) `docs/observability/PHASE2_ALERTS.md` 10 alerts+auto-responses. (C) `RELEASE_STATE.yaml` machine-readable state. (D) `docs/runbooks/01..08`. (E) `apps/web/src/lib/translation/lifecycle.ts` (pure planners, NO destructive migration) + `docs/observability/PHASE2_RETENTION.md`. (F) Independent audit of Agent 1-3.

AUDIT FINDINGS (no blockers): (1) transition_translation_order resets its txn-local GUC `set_config(...,'off',true)` immediately after the UPDATE; if the UPDATE raised, the GUC stays 'on' but the txn rolls back → safe. (2) claim_outbox_event uses FOR UPDATE SKIP LOCKED + UNIQUE idempotency_key → exactly-once, no double-send. (3) document_artifacts immutability triggers (UPDATE/DELETE rejected) + content-addressed storage key (`<orderId>/<sha>.pdf`, upsert idempotent) prevent overwrite; corrections are new versions. (4) requireTranslationOperator() is first call in every action, fail-closed when ADMIN_SECRET unset → no authz bypass. (5) PII discipline holds: recipient only Stripe-derived, outbox holds opaque recipient_ref hash, logs carry codes/ids only. (6) Migrations are forward-only additive; rollback = drop the _v2 objects (legacy translation_orders untouched). MINOR: webhook amount/price-mismatch + payment_to_order_latency events are DEFINED but not yet wired into the legacy webhook (V2 payment→order coupling is owner-side) — documented in PHASE2_ALERTS.md gauge section.

NEXT TASK (coordinator): owner review before any flag flip / merge. To activate events set `PHASE2_EVENTS_ENABLED=1` (PII-safe, log-only). Wire the V2 webhook→order coupling to enable amount/price-mismatch + payment_to_order_latency emits. Consume lifecycle deletion plans from a future cron (no destructive migration shipped). Defer hosted Stripe positive E2E to final acceptance (owner). Prod stays SHADOW; PR #119 DRAFT; do NOT merge.

EVIDENCE: input_sha=44ec7b8fcaa61ac621dbb1f2a8e14b64ea8eab44. tsc 0 errors; full vitest 3764 pass / 38 skip / 0 fail (37 new across 2 files: events PII-gate, lifecycle). No prod env/flag/Vercel change; PHASE2_EVENTS_ENABLED OFF.

---
# HANDOFF (2026-06-14 — Phase 2 Agent 2: Translation Operator Pipeline V2 application flow)

DONE: Wired the application flow onto Agent 1's DB foundation (branch `architecture/translation-operator-canonical-v2`, input HEAD f383cb8). (A) Refactored `submit-order/route.ts` to the V2 contract — Stripe-verified recipient only, client field values non-authoritative, canonical load+product+session-ownership+hash verification (404/403/403/409), idempotent `createOrGetOrder`+`bindCanonicalDocument`+queued transition, legacy `manual_review_queue` row STILL written with v2 linkage, gated by `getCanonicalMode('translation')` (off→legacy-only; enforce→canonical required). (B) New `lib/auth/requireTranslationOperator.ts` (independent admin_session cookie===ADMIN_SECRET, fail-closed) called first in all 8 V2 actions (`[id]/v2Actions.ts`) + retrofitted into the 3 legacy actions; every action asserts expected version+status; recipient change is a separate audited action. (C) `[id]/v2/page.tsx` operator view from resolved canonical + overrides + events + artifacts (not mutable source_fields). (D) `lib/translation/orders/renderFromCanonical.ts` renders ONCE from resolved canonical (no fabricated fields, C3-null omitted), computes 7-field certification + byte SHA-256, uploads to private bucket, `create_artifact_and_enqueue`. (E) `api/internal/translation-delivery/route.ts` worker — CRON_SECRET auth, claim_outbox_event exactly-once, exact stored bytes (hash-verified, no re-render), resend idempotency_key, success/transient-retry/permanent-fail transitions. Added `getArtifactById`/`downloadArtifactBytes`/`markOutboxPermanentlyFailed` to the data module; added optional `idempotencyKey` to `sendEmail`.

NEXT TASK (coordinator): owner review before any flag flip. To activate V2 fully set `CANONICAL_MODE_TRANSLATION=enforce` (or shadow to dual-run). Wire a Vercel cron to POST `/api/internal/translation-delivery` with the CRON_SECRET Bearer. Point the wizard's post-checkout call at submit-order with `canonical_document_id`. Add a queue-list link to the `/v2` view. Do NOT mutate legacy `translation_orders`; prod stays shadow until owner approval.

EVIDENCE: input_sha=f383cb878a2a80373939e6111a1e6c2bb5973587. tsc 0 errors; full vitest 3727 pass / 38 skip / 0 fail (37 new across 4 files: renderFromCanonical, submitOrderV2, operatorAuthAndActions, deliveryWorker); build PASS incl. new routes `/api/internal/translation-delivery` + `/admin/manual-review/[id]/v2`. No merge, no Vercel/env change, enforce not enabled.

---
# HANDOFF (2026-06-14 — Phase 2 Agent 1: Translation Operator Pipeline V2 DB foundation)

DONE: Built the Phase 2 DB foundation on branch `architecture/translation-operator-canonical-v2`. THREE additive forward-only migrations APPLIED to prod `rtfxrlountkoegsseukx` + written as files under `supabase/migrations/20260614000001..3`. (1) `translation_orders_v2` (durable order; checkout_session_id UNIQUE; optional canonical FK; product='translation' CHECK; version; legacy flag; timestamps) + `translation_order_events` (append-only). State machine RPC `transition_translation_order` (advisory-locked, expected status+version under lock, allowed-transition map, actor required) + BEFORE UPDATE guard (status/version change ONLY via RPC through txn-local GUC; version decrement blocked; checkout immutable; canonical rebind forbidden) + append-only event triggers. (2) `document_artifacts` (immutable, hash-bound, UNIQUE(order_id,artifact_version)) + `delivery_outbox` (idempotency_key UNIQUE) + `create_artifact_and_enqueue` (atomic artifact+2 transitions+outbox) + `claim_outbox_event` (FOR UPDATE SKIP LOCKED) + private bucket `translation-artifacts` + `phase2_admin_cleanup`. Widened `canonical_overrides_source_check` to admit 'operator_override' (additive). (3) Widened canonical immutability guards to also release for PHASE2_TEST_ sentinel cleanup. TS module `apps/web/src/lib/translation/orders/index.ts` + tests (`phase2DataModel.live.test.ts` 14 live, `orderErrors.unit.test.ts` 7 unit).

NEXT TASK (Phase 2 Agent 2+ / coordinator): wire `submit-order` route to create a `translation_orders_v2` row (carry canonical_document_id when present; verified_recipient_email from Stripe only); build the operator UI/API that drives transitions + applyOperatorOverride; build the delivery worker that calls claimOutboxEvent → send email → markOutboxDelivered + transition delivery_pending→delivered. Do NOT mutate the legacy `translation_orders` table. No prod env/flag flip required for the foundation.

EVIDENCE: input_sha=bd986677b1e0743b86246bcc6ae6947ce62de84f. tsc 0 errors; unit 7/7; live DB invariants 14/14 (synthetic PHASE2_TEST_ data, cleaned to 0 leftover; legacy translation_orders still 2 rows). Supabase security advisors: 0 new findings on Phase 2 objects (only pre-existing guard_block_events/is_admin/is_moderator_or_admin remain).
