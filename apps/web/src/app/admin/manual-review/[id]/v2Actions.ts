 ...
 *
 * PII: never log field values or raw emails. Actor is opaque ('operator:admin').
 */
...
export async function assignOrder(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const version = await doTransition(order, 'assigned', actor.id, 'operator_assign')
...
export async function beginReview(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const expectedVersion = parseVersion(formData)
    const order = await loadAtVersion(id, expectedVersion)
    const version = await doTransition(order, 'in_review', actor.id, 'operator_begin_review')
    revalidatePath(`/admin/manual-review/${id}`)
    return { ok: true, version }
  } catch (e) {
    return fail(e)
...
export async function requestClarification(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const order = await loadAtVersion(id, expectedVersion)
    const version = await doTransition(order, 'needs_user_clarification', actor.id, 'operator_request_clarification', {
      has_reason: !!reason,
    })
    revalidatePath(`/admin/manual-review/${id}`)
    return { ok: true, version }
...
export async function approveForRender(formData: FormData): Promise<ActionResult> {
  try {
    ...
    if (order.status !== 'approved_for_render') {
      const v = await doTransition(order, 'approved_for_render', actor.id, 'operator_approve')
...
export async function retryDelivery(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const version = await doTransition(order, 'delivery_pending', actor.id, 'operator_retry_delivery')
    revalidatePath(`/admin/manual-review/${id}`)
    return { ok: true, version }
  } catch (e) {
    return fail(e)
...
export async function cancelOrder(formData: FormData): Promise<ActionResult> {
  try {
    ...
    const order = await loadAtVersion(id, expectedVersion)
    const version = await doTransition(order, 'cancelled', actor.id, 'operator_cancel', { has_reason: !!reason })
    revalidatePath(`/admin/manual-review/${id}`)
    return { ok: true, version }
  } catch (e) {
    return fail(e)
