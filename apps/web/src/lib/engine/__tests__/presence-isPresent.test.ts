...
describe('#21 — word-aware isPresent', () => {
  const page = ocr("Куроп'ятник Сергій Сергійович 25 червня 1986 смт Тростянець Вінницька область Централізовано видано")
  ...
  it('confirms a multi-word value when every word is present', () => {
    expect(isPresent("Куроп'ятник Сергій Сергійович", page)).toBe(true)
  })
  it('does NOT confirm a prefix of a longer word (куроп ⊄ куропятник)', () => {
    expect(isPresent('Куроп', page)).toBe(false)
  })
