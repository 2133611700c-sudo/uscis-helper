...
const FIX = path.join(ROOT, 'test-fixtures/real-docs/military_id_p1_redacted.jpg')
...
describe.skipIf(!LIVE)('LIVE pipeline E2E (military ID, owner fixture)', () => {
  ...
  it('MILITARY ID — Cyrillic → KMU-55 + registry settlement (with type)', async () => {
    const f = await run('military_id_p1_redacted.jpg', 'ua_military_id')
  ...
  it('PASSPORT — MRZ controlling Latin beats re-transliteration', async () => {
    const f = await run('internal_passport_redacted.jpg', 'ua_international_passport')
    expect(f['family_name']?.latin || '').toMatch(/REDACTED/i)   // MRZ, not KMU-55 guess
    expect(f['passport_number']?.latin || '').toMatch(/PASSPORT-REDACTED/i)
    expect(f['date_of_birth']?.latin || '').toMatch(/1986/)
  }, 120000)

  it('BIRTH CERT (handwritten 1986) — reads name + place, flags review', async () => {
    const f = await run('birth_cert_soviet_redacted.jpg', 'ua_birth_certificate')
