...
describe('isStrictValidValue — passport_number', () => {
  it('accepts Ukrainian international + booklet formats', () => {
    expect(isStrictValidValue('passport_number', 'PASSPORT-REDACTED')).toBe(true)
  ...
  it('rejects raw OCR garbage masquerading as a passport number', () => {
    expect(isStrictValidValue('passport_number', 'Passport No: PASSPORT-REDACTED')).toBe(false)
...
describe('normalizeAndValidate — combined pipeline', () => {
  ...
  it('preserves canonical inputs unchanged', () => {
    ...
    expect(normalizeAndValidate('passport_number', 'PASSPORT-REDACTED')).toEqual({ ok: true, value: 'PASSPORT-REDACTED' })
  })
