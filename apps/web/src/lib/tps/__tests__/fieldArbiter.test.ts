...
describe('Field Arbiter v0', () => {
  ...
  it('resolveAllFields: full packet resolution', () => {
    const result = resolveAllFields({
      uploads: {
        passport: [
          { field: 'family_name', value: 'Kuropiatnyk', sourceDoc: 'passport', sourceType: 'ocr_mrz', confidence: 0.99, reviewRequired: false },
          { field: 'given_name', value: 'Sergii', sourceDoc: 'passport', sourceType: 'ocr_mrz', confidence: 0.99, reviewRequired: false },
  ...
  it('same value from multiple sources: no conflict', () => {
    const candidates: ExtractedCandidate[] = [
      { field: 'family_name', value: 'Kuropiatnyk', sourceDoc: 'passport', sourceType: 'ocr_mrz', confidence: 0.99, reviewRequired: false },
      { field: 'family_name', value: 'KUROPIATNYK', sourceDoc: 'i94', sourceType: 'ocr_keyword', confidence: 0.9, reviewRequired: false },
    ]
    const result = resolveField('family_name', candidates)
    expect(result.chosenValue).toBe('Kuropiatnyk')
