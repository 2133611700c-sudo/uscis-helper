...
describe('certification PDF — zero-trust output', () => {
  ...
  beforeAll(async () => {
    buf = await generateTranslationPDF({
      ...
      fields: [ef('surname', 'REDACTED')], sourceTraces: [],
      certificationRecord: {
        signer_full_name: 'Serhii REDACTED', address: '1213 Gordon St, Los Angeles, CA 90038',
        language_pair_confirmed: true, statement: '', signature_typed_name: 'Serhii REDACTED',
  ...
  it('contains Name, Address and Date', () => {
    expect(text).toContain('Serhii REDACTED')
