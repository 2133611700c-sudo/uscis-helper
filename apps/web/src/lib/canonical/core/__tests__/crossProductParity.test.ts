...
describe('cross-product canonical parity — identity keys are identical across consumers', () => {
  ...
  it('KNOWN PARITY FAILURE: Re-Parole RESURRECTS the C3-rejected value (REVIEW_LOST)', () => {
    // reParoleAdapter.getValue ignores finalValue and reads normalizedValue ?? rawValue,
    // so the field C3 deliberately rejected (finalValue=null) is released anyway.
    // This is the documented "Re-Parole blind spot". We PIN the current (buggy)
    // behavior so a silent change is caught. Contract-correct behavior is: null.
    // DO NOT change this expectation to make a fix look done — fix reParoleAdapter
    // to use getCanonicalValue(), then flip this to toBeNull().
    expect(reparole.country_of_nationality).toBe(REJECTED_NORM) // BUG, pinned
    // The other three honor the rejection — proving the divergence is Re-Parole-only.
    expect(ead.country_of_nationality).toBeNull()
  })
