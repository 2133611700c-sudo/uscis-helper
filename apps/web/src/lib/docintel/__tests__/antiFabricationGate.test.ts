...
describe('applyAntiFabricationGate (pure)', () => {
  it('handwritten birth cert: forces review on identity, keeps value, adds reasons', () => {
    ...
    expect(name.value).toBe('REDACTED') // value unchanged
  ...
  it('non-hard-case (passport): fields untouched (MRZ identity not blanket-forced)', () => {
    const input = [
      field({ field: 'family_name', value: 'REDACTED', review_required: false }),
      field({ field: 'passport_number', value: 'PASSPORT-REDACTED', review_required: false }),
    ]
  ...
  return {
    ...
    async readFields(): Promise<VisionReadResult> {
      return {
        ...
        fields: [
          { field: 'child_family_name', cyrillic: 'REDACTED_NAME', can_read: true, confidence: 0.99, reason: '' },
