...
describe('disabled → byte-identical (no mutation, no block)', () => {
  it('returns fields untouched when ctx.enabled is false', () => {
...
describe('finalize — TIER 1 source_verified', () => {
  it('sets final_value and clears the review flag (certifier attested)', () => {
    const fields: FieldWithMaybeOverride[] = [
...
describe('user_confirmed alone on a TIER 1 field → block (422)', () => {
  it('rejects user-alone on critical identity', () => {
    const fields: FieldWithMaybeOverride[] = [
...
describe('anchor conflict → block', () => {
  it('blocks when the proposed value disagrees with the cross-document anchor', () => {
    const fields: FieldWithMaybeOverride[] = [
