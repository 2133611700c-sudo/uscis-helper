...
describe('Phase 2 — normalizeName', () => {
  ...
  it('normalizes REDACTED → REDACTED', () => {
    expect(normalizeName('REDACTED')).toBe('REDACTED')
  })
...
describe('Phase 2 — analyseNameField', () => {
  ...
  it('does NOT flag normal REDACTED as review_required', () => {
    const result = analyseNameField('REDACTED')
    expect(result.review_required).toBe(false)
  })
