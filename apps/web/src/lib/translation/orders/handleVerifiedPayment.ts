 ...
 *   - Stripe EVENT id   → webhook-processing dedupe (stripe_processed_events; webhook path only).
 *   - checkout_session_id → order uniqueness (translation_orders_v2 UNIQUE; both paths).
 * Duplicate webhook → same order; client-after-webhook → same order; webhook-after-client → same
 * order; concurrent → same order; client-never-returns → webhook still creates. No second audit
 ...
 * the customer. It only creates-or-gets the order, binds canonical immutably (or leaves it for the
 * operator), and lands the order in its initial paid/queued state.
 *
 * ── PII ──────────────────────────────────────────────────────────────────────────────────────────
...
export interface HandleVerifiedPaymentInput {
  ...
  /** The Stripe event id (webhook path) or null (client reconciliation has no event). */
...
export async function handleVerifiedPayment(
  ...
  // 4. Create-or-get the order idempotently on checkout_session_id.
  let order
  let created: boolean
  try {
