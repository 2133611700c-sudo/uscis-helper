  ...
  return (
    <div className="w-full bg-brand-50 border-b border-brand-100">
      <div className="max-w-[1200px] mx-auto px-4 md:px-6 py-2 flex items-center gap-3 overflow-x-auto scrollbar-none">
        <span className="flex items-center gap-1.5 text-xs font-semibold text-brand-700 shrink-0">
        ...
        <div className="flex items-center gap-2 flex-nowrap">
          {trendingTopics.map((topic) => (
            <Link
              ...
              className="shrink-0 inline-flex items-center px-3 py-1 rounded-badge bg-white border border-brand-100 text-xs font-medium text-brand-700 hover:bg-brand-100 hover:border-brand-200 transition-colors whitespace-nowrap"
            >
              {t(`topics.${topic.id}`)}
            </Link>
          ))}
