...
- New `generatePdfRoute.test.ts` (in-memory; canonical/owner/Stripe/pdf/email mocked): owner happy path → PDF + persisted order+audit rows via repository + email; 400 fields_require_review; 402 no-token.
- **RATCHET → 0.** `noDirectSupabaseInDomain.test.ts`: `KNOWN_COUPLED_ROUTES` emptied; final assertion flipped to require ZERO translation routes importing Supabase directly (`toEqual([])` + `size===0`). The route layer is now fully repository-driven; Supabase access lives ONLY in the not-connected adapter stub.
- **FULL SUITE GREEN: 4944 passed / 26 skipped / 0 failed; tsc 0; PII clean.** All flags default OFF; Supabase NOT connected (in-memory default). **APPLICATION CODE COMPLETE for routes — READY FOR SUPABASE ADAPTER CONNECTION + DATABASE-BACKED STAGING VALIDATION.** NOT production-ready (no DB-backed E2E / owner sign-off).
...
- **80 contract+pdf tests green; full suite 4747 passed; tsc 0.** Three label islands (PDF English + review Ukrainian + schema) now single-sourced via the contract with zero behavior change. Not committed/pushed (push auto-deploys; flag stays OFF). Plan: `docs/architecture/contracts/ua_birth_certificate_soviet_v1/migration-plan.md`.

## 2026-06-27 | UA-TrOCR last-lever tested (FAIL) + Unified Document Contract Phase 2 reconcile
- **UA-TrOCR (cyrillic-trocr/trocr-ukrainian-handwritten) downloaded externally + tested** on the committed cross-hand harness (same frozen boxes). Verified: checkpoint 1,335,747,032 B, sha256 `cad9e6b33329…`, HF repo revision `60429f5bcfd5d78f06aa9b19ec1237a4fe03d8fb`, safe_open 480 tensors, offline load OK (334M params). Result: **hand A 0/3 strict** (folded-soft 2/3), **hand B 0/3 strict** (the model's own UA target). Scorer independently validated (2-run determinism, separate-Levenshtein CER match to 1e-9, verdict 6/6, fold-fairness Cyrillic output, 0 infra errors). Frozen PII-safe evidence → `docs/research/UA_TROCR_TIERA_EVIDENCE.md`. Status (scoped, NOT overgeneralized): `OFF-THE-SHELF HTR SEARCH FOR MVP: CLOSED`; `GENERAL ZERO-SHOT HTR IMPOSSIBILITY: NOT PROVEN`; `AUTO-FINALIZATION: PROHIBITED`.
- **Unified Document Contract — Phase 2 (reconcile) DONE.** New `apps/web/src/lib/contracts/__tests__/birthCertSovietV1Reconcile.test.ts`: asserts the contract does not contradict the second alias island `KEY_ALIASES` (D), covers the C crop layer `FIELD_BOX_TEMPLATES`, and **pins the latent HTR empty-intersection bug** (audit conflict #1 — A `child_*` keys never match `HTR_NAME_FIELDS`; the contract bridges `child_family_name→family_name`). Additive test only, no runtime change.
- **Unified Document Contract — Phase 3 (alias maps DERIVE from contract) IMPLEMENTED + VERIFIED.** `buildMirrorValues.ts` `resolveAliases()` and `keyAliases.ts` `resolveKeyAliases()` are now flag-gated (`UNIFIED_DOC_CONTRACT_ENABLED`, **default OFF = legacy literals, byte-identical**); flag ON sources the birth-cert maps from the contract via `birthCertMirrorAliases()` / `birthCertContractKeyAliases()`. Parity proven by `phase3ContractRuntimeParity.test.ts`: `birthCertMirrorAliases()` deep-equals legacy `ALIASES.ua_birth_certificate`; `buildMirrorValues`/`collectMirrorExtras` identical OFF vs ON on a fictional fixture; `resolveKeyAliases(ON)===OFF`; contract contribution a strict subset of legacy. **633 contract+mirror+canonical tests green, tsc 0.** Two alias islands collapsed to one source with zero behavior change. (Code+parity test were present in the working tree; this session verified parity holds by reading the assertions + running the suite, not trusting a green check.) NOT committed/pushed (push auto-deploys; flag stays OFF until owner flips in Phase 4).
...
- **The three OWN bugs that caused the false 0/8:** (1) field crops **downscaled to height 128** → cursive signal destroyed [BINDING root, agent 3]; (2) scorer folded Latin output vs Cyrillic GT (`\w` keeps a–z) → correct reads scored as misses [agent 1]; (3) loose crops carried printed-label + stamp contamination, read as extra chars (CER>1) [agent 2]. Binarization HURTS (Otsu 0.30, Sauvola 0.90). Data was NOT the constraint (agent 4 superseded — reasoned from old low-res numbers); synthetic-font fine-tuning abandoned (synth val 0.59→0.26 but real flat/worse).
- **Standard set:** NEW `docs/adr/ADR-026-htr-native-res-pipeline.md` — production rule: crop each field at NATIVE resolution, contrast-stretch only, never downscale/binarize, channel-aware scoring, `raxtemur` reader, human-review stays. Corrected `docs/architecture/MODEL_INVENTORY.md` blanket claim + `docs/research/HTR_ZEROSHOT_POC.md` verdict (correction banner + ROOT-CAUSE REVERSAL section). Scope = modern UA & RU Cyrillic.
- **Open:** re-test LLM APIs on native-res crops (their "fabrication" may also be low-res artifact); automatic per-field region localization; broader-N validation before relaxing human-review gate. Scripts/crops gitignored; PII guard clean.
...
- **BAKE-OFF COMPLETION (same day, after expert critique):** tested the missing local candidate + the image-hypothesis. (A) `achimrabus/crnn-ctc-ukrainian` (Apache, PyLaia CRNN-CTC, loaded via polyscriptor's MIT `inference_pylaia_native.py`) — surname CER 0.90/conf 0.33, given CER 1.00 = WORSE than TrOCR; `crnn-ctc-church-slavonic` CER 1.0–1.17 (transfer-gap confirmed). PyLaia gives LOW conf (0.21–0.38) on wrong reads → usable reviewer-assist flag. (B) blue registry stamp = only ~5% of crop; removing it moved raxtemur surname CER 1.10→0.70 (partial) → dominant blocker is the cursive itself, not occlusion. **Decisive: NO key-free model (TrOCR transformer / PyLaia CRNN / UA-specific) reads the cursive surname zero-shot — off-the-shelf ceiling, not a model-selection miss.** License: card Apache ≠ train-data cleared → benchmark-only until rights audit; cloud comparison readers NOT run (PII egress).
- **Files:** NEW `docs/research/HTR_ZEROSHOT_POC.md` (PII-free, with bake-off section). Real reads/crops/venv/polyscriptor clone stay gitignored in `qa-private/`. PII guard clean 0/1802.
...
- **Corrected EVERYWHERE (authoritative → references):**
  - `modelMatrix.ts` (LAW): FALLBACK_MODELS now `[2.5-pro, 3.5-flash, 2.5-flash]`; 2.5-pro added to DISQUALIFIED for certs; NEW `MODEL_PROFILES` typed inventory + `HANDWRITTEN_DOC_FAMILIES`/`isHandwrittenFamily()`. modelMatrix.test 11/11.
  - `geminiVisionProvider.ts`: `maxOutputTokens` → `max(8192, GEMINI_MAX_OUTPUT_TOKENS||16384)` (fixes thinking-model MAX_TOKENS empties — affects the primary too).
  - `vision-extract/route.ts`: empty-case model report fixed (primary, not a flash literal).
  - NEW `docs/architecture/MODEL_INVENTORY.md` (human inventory) + agents updated ADR-018, CONSTITUTION L1, root CLAUDE.md, RECOGNITION_ORG_CHART (fixed the WRONG "Gemini 2.5 Pro Vision" reader), RULES_MASTER_INDEX.
- **Handwriting LAW (new):** no model — not even primary — is proven error-free on handwritten certificates → ALWAYS human-reviewed regardless of model.
...
- **Proof the connected FREE pipeline works (zero Gemini):** the owner's Russian birth cert, fed as Cyrillic, deterministically produced the full English certified-translation output — Birth Certificate; Solovyak/Andrey/Timofeyevich; SEX=M (derived from отчество, was a MISS); parents; urban-type settlement Trostyanets, Vinnytsia Oblast; II-БК № 530174. All from the codex, $0.
...
- The stability audit found sex=MISS on the birth cert + military ID. The patronymic encodes sex unambiguously (Тимофеевич→M, Петровна→F), so derive it for $0 — the cost-efficiency-first principle (never spend an LLM call for what a suffix tells us). This is the teaching loop in action: mentor found the gap → encoded a deterministic FREE rule in the ONE codex.
- `packages/knowledge` (codex): new `sexFromPatronymic(p): Sex|null` — female suffixes checked first (longer/more specific); not-a-patronymic ⇒ null (never guesses). Exported from index. +16 tests (sexFromPatronymic.test, wired into CI). Verified live before pinning (12/12 incl. owner's Тимофеевич→M).
...
- **PHASE 2 — ONE source → both models (docReadingRulesSync.test, 17):** guard that every non-image-only rule taught to Gemini (readingRulesPromptBlock) also reaches DeepSeek (textRulesForDeepSeek); RU rules confirmed in both. New exported isImageOnlyRule keeps the guard in lock-step with the real drop logic. Closes teaching GAP-3.
- **PHASE 2/3 — teaching ledger + process:** docs/codex/RULE_REGISTRY.md (rule→source→taught-to→measured) + docs/codex/TEACHING_LOOP.md (mentor reads real doc → encode once in the ONE codex → auto-teach both models → measure FREE first, paid only when budget allows, 429→BLOCKED). Cost ordering: fill empties from FREE sources (cross-doc/MRZ/patronymic/dictionary) BEFORE any paid Gemini call.
- **PHASE 4 — provenance:** civil_registry_terms.json schema_version 1.0 + _sources (143-ФЗ, BGN/PCGN, GeoNames). **PHASE 5 — ADR-025** records the rejected heavy infra (DSPy/Langfuse/LangGraph/Guidance/fine-tuning/RAG/MT-swap) with reasons.
- Evidence: knowledge suites green (reference 29, RU glossary 55, alphabet 82, …); apps/web 4646 pass; tsc 0. ZERO new runtime deps; ZERO Gemini calls (all validation deterministic). Saved cost-efficiency-first principle to memory.

## 2026-06-23 | Full Russian-language document support: RU→EN glossary + places + a separate Russian rule
- Practice: Ukrainian/Soviet documents are often written in RUSSIAN. Added complete RU→ENGLISH support, sourced from the RU normative base (Federal Law 143-ФЗ «Об актах гражданского состояния»; BGN/PCGN; ICAO/passport-match via L7). Research web-cited. ONE codex (L2) — all in packages/knowledge.
- **Places (dictionary.ts, directly wired):** all 24 Ukrainian oblasts in RUSSIAN genitive+nominative → English (Винницкой области / Винницкая область → Vinnytsia Oblast — modern Ukrainian form, NOT Russified "Vinnitsa"); RU oblast suffix «области» stripped; RU settlement designators (посёлок городского типа/пгт → urban-type settlement, деревня/село → village, город/г. → bare city). Verified live: Винницкой области→Vinnytsia Oblast, Харьковской→Kharkiv, пгт/деревня correct.
- **Terms (civil_registry_terms.json, lang: ru):** Russian document headers (СВИДЕТЕЛЬСТВО О РОЖДЕНИИ→Birth Certificate, …О БРАКЕ/РАСТОРЖЕНИИ/СМЕРТИ/ПЕРЕМЕНЕ ИМЕНИ), RU registry agencies (ЗАГС/отдел/орган ЗАГС→Civil Registry Office), RU field labels (фамилия/имя/отчество→Surname/Given/Patronymic; отчество≠Middle Name), and a Soviet-abbreviations block (СССР/РСФСР/УССР/МВД/ОВД/военный комиссариат→USSR/RSFSR/Ukrainian SSR/…).
- **Separate explicit Russian rule (docReadingRules.ts):** new RUSSIAN_DOCUMENT_RULE (distinct from the reading-only RUSSIAN_SCRIPT_RULE) — RU→EN term/place rendering, отчество=Patronymic, Ukrainian-form places, passport-match for names, archaic pre-1918 letters (ѣ/і/ѳ/ѵ→modern). Injected into the 4 RU doc classes; flows to DeepSeek via textRulesForDeepSeek (L9).
- **Tests:** new russianGlossary.test.ts (55 — oblast/settlement/terms/transliteration + golden RU→EN vector for the owner's birth cert), wired into the knowledge test script; updated dictionaryAutocorrect (RU oblast now deterministic, an improvement). Transliteration already 33-letter complete (BGN/PCGN). Suite 4622 pass; tsc 0. BGN/PCGN kept (controlling-Latin override intact); flag-OFF byte-identical.

## 2026-06-23 | Honest multi-run stability audit on real docs + orientation K-vote (owner distrust vindicated)
- Built `stability-audit.mjs` (N runs/doc, majority-vs-GT, per-field variance, BLOCKED-not-faked) and ran it N=5 on the 3 real identity docs. Report: docs/reports/STABILITY_AUDIT_2026-06-23.md.
- **The owner was RIGHT to distrust single-run proofs.** Findings: identity Cyrillic read STABLE (5/5); but (a) tile recovery flaky at N=5 (passport 4/5, birth 2/4) — K-sample voting FIXED it (passport→5/5, birth→4/5); (b) the grid orientation detector FLIPS 0/270 on the birth cert (I had wrongly claimed 3/3 stable) → added detectUprightCwVoted (ORIENT_VOTE_RUNS=3, foldOrientationVotes strict-majority, split→no-rotate, +8 tests); (c) the combined K3×K3 voted run burst-tripped Google rate limits (HTTP 503 + flash fallback) → the audit honestly reported 0 ok / 0 faked (ADR-018 gate proven).
- **Honest caveats:** standalone readDocument accuracy (66.7%@majority) is NOT a clean product number (no MRZ injection; intl-passport fixture scored as booklet) — real accuracy needs the live route. Real gap: sex not extracted on birth/military (derivable from patronymic).
- All flags default OFF (byte-identical). Suite 4622 pass; tsc 0. Next (owner-gated): re-measure combined voting after cooldown; vote-on-doubt orientation; sex-from-patronymic.

## 2026-06-23 | Stabilize tile recovery with K-sample MAJORITY VOTING (research-backed; fixes the 0/4/7 качели)
- **Owner distrust addressed at the root:** the STAGE-4 hi-res tile recovery was flaky (same crop gave 0/5, 4/5, 7/7 across runs) — a single temperature-0 VLM sample is non-deterministic on dense handwriting. 2025-2026 research (Structured Self-Consistency; Consensus-Entropy CE-OCR +42% F1) says the proven fix is multi-sample + majority voting.
- `ensemble/tileRegionRead.ts`: kept the single call as `geminiReadFieldsFromCropOnce`; the production `geminiReadFieldsFromCrop` now reads each crop K times (env `TILE_VOTE_RUNS`, default 3, clamp 1..5) and `foldMajority`s — per field, normalize-then-count (shared `normalizeForCompare`), a value WINS only on a STRICT majority (bestCount*2 > runs ⇒ ≥2/3, ≥3/5), emits the exact raw winning string; NO majority ⇒ field OMITTED (stays empty, held for review) so a lone flaky read never wins. Same positional signature ⇒ the documentFieldReader caller is unchanged; runs=1 ⇒ byte-identical to the old single read. Stays inside HIRES_TILE_RECOVER_ENABLED; fail-open (a thrown sample = no vote).
- **Tests:** +11 deterministic (injected sampler, no Gemini): 2/3 wins, 1-1-1 omits, normalize-then-vote, 2-2-1/5 omits, 3/5 wins, lone-read omits, wrapper passes only majority keys, per-sample-throw still majority, runs:1 back-compat, env parsing, integration through recoverEmptyFieldsByTiles (majority filled, no-majority held). Suite 4614 pass (+11); tsc 0. Flag OFF + TILE_VOTE_RUNS=1 byte-identical.

## 2026-06-22 | STAGE 4 DONE: hi-res tile recovery WIRED into all 4 OCR routes (end-to-end proven 7/7)
- Finished STAGE 4: `readDocument` now runs `recoverEmptyFieldsByTiles` behind HIRES_TILE_RECOVER_ENABLED (default OFF) using a new `opts.originalBuffer` (the pre-downscale high-res upload). It orients the original with the reliable content grid detector, then re-reads empty CRITICAL fields (handwritten/required) from overlapping hi-res L/R tiles via `geminiReadFieldsFromCrop`. Single integration point in readDocument (uniform for all products); each of the 4 OCR routes adds one arg `originalBuffer: rawBuffer`.
- **END-TO-END LIVE PROOF** (`scripts/test-wired-recover.mjs`, flag ON, real readDocument): birth cert recovered **7/7** empty fields, all matching GT — father "Соловьяк Андрей Богданович", mother "Соловьяк Дарья Петровна", series "II-БК № 530174", office "Тростянецкий райотдел ЗАГСа", + act/date/issue. All held for review, raw_cyrillic only (C3/transliteration own the canonical value), tagged `hires_tile_recovered`.
- Reliability fixes found live: the dense-cursive crop read needs a 60s timeout (30s aborted) + 3 retries (transient socket close on this local network — prod Vercel→Google is steadier); the preview model wraps JSON in ```json fences (salvage-parse).
- **Tests:** tileRecoverWiring.test.ts (8: readDocument imports + flag/originalBuffer gate + empty-critical short-circuit + orients original + opt declared; all 4 routes pass originalBuffer: rawBuffer). Suite 4603 pass / 24 skip; tsc 0. Flag OFF ⇒ byte-identical (block skipped). Routes pass the original unconditionally but readDocument only tiles when the flag is on AND criticals are empty.

## 2026-06-22 | STAGE 4: hi-res tile recovery of empty fields — root cause = RESOLUTION (proven + fixed live)
- **Owner task #19:** the Gemini pipeline returned EMPTY for fields that ARE present (birth-cert father/mother/act/series/issuing-office; passport printed dob/place/dates). Investigated + verified myself with live paid Gemini.
- **🔴 Root cause CONFIRMED = resolution, not field-spec/prompt:** at full-page scale (preprocess downscales long side to 2048) a dense handwritten region is ~4-5 px/letter → Gemini honestly returns can_read=false. A HIGH-RES crop (~2×) of the right page READ them — proven live: father "Соловьяк Андрей Богданович", mother "Соловьяк Дарья Петровна", series "II-БК № 530174", office "Тростянецкий райотдел ЗАГСа". Two things are BOTH necessary: hi-res crop AND a targeted aggressive prompt ("read these named fields, never empty for a present field") — the standard full-page prompt on a tile stays conservative.
- **New module `docintel/ensemble/tileRegionRead.ts` (STAGE 4, generalizes the date-only `dateRegionRead`):** when the main read leaves critical fields empty, split the high-res UPRIGHT image into overlapping L/R tiles and read the empties with `geminiReadFieldsFromCrop` (targeted, plain-text+salvage parse [the preview model wraps JSON], retry on socket close). Additive + fail-open: only empties pursued, confidently-read fields never touched, raw_cyrillic only (C3/transliteration own the canonical value), held for review (reason `hires_tile_recovered`). Does NOT apply EXIF (the orientation stage runs first; EXIF is unreliable). Behind `HIRES_TILE_RECOVER_ENABLED` (default OFF). Crop reader injected ⇒ unit-testable.
- **LIVE PROOF:** `scripts/test-tile-recover.mjs` — recovered **4/5** empty critical fields, ALL matching owner GT (only act_record_number left, which the owner marks model-unstable 87/84). Cost ~$0.002/tile.
- **Tests:** tileRegionRead.test.ts (8: isEmptyField, fill-from-tile [held+reason+raw-only], early-stop after all recovered, no-empty⇒no-tiles, criticalKeys gating, fail-open, flag OFF). Suite 4595 pass / 24 skip (+8); tsc 0.
- **NOT wired into the routes yet** (engine-only, like crossDocReconcile was): integration point is the OCR route (it holds the pre-downscale original image) — after readDocument, if empties + flag on, call recoverEmptyFieldsByTiles with the original hi-res oriented buffer + spec field labels.

## 2026-06-22 | Orientation-first: reliable CONTENT-based upright detector (grid compare) — replaces unreliable detectCw/EXIF
- **Owner rule:** detect document orientation FIRST + accurately (real owner docs are photographed sideways). Verified everything myself on the REAL docs with live Gemini ($9 paid key under GEMINI_API_KEY_PAY; bare GEMINI_API_KEY is dead free-tier).
- **🔴 Root cause proven:** EXIF orientation is UNRELIABLE — the military ID and birth cert carry the SAME EXIF flag 6, but the military is genuinely sideways (needs it) while the birth cert is already upright (EXIF BREAKS it → preprocessImage's blind sharp.rotate() turns the upright birth cert sideways). The old `autoOrient` detectCw ("how many degrees CW") was also unreliable: false-negatived the sideways military (said 0°) and false-positived the upright birth cert (said 270°). Validated visually (my own eyes on rendered rotations).
- **New module `detectOrientation.ts`:** renders the page at all 4 rotations into ONE 2×2 grid and asks Gemini in a SINGLE call which cell is upright (direct comparison ≫ absolute-angle guess). Live result: **3/3 correct + STABLE** across reruns, on the post-preprocess buffer (passport 0°, military 0° [EXIF already fixed it], birth 270° [corrects the EXIF mistake]). `buildOrientationGrid` + `positionToCorrectionCw` + `orientToUpright` (fail-open) + `detectUprightCw`. Behind `CONTENT_ORIENT_ENABLED` (default OFF); wired into documentFieldReader as the PREFERRED path over the deprecated autoOrient.
- **HONEST limit (measured, not assumed):** correct orientation did NOT recover the birth cert's empty father/mother/series/authority fields (those are PRESENT in the doc but Gemini returns empty even upright) — that is a SEPARATE recognition gap, not orientation. And for docs Gemini already mentally-rotates, correcting orientation gave ~no recognition lift (slight place regression on birth). The detector's value is RELIABILITY/SAFETY (it stops EXIF from breaking an upright doc) — foundational correctness, measured before any flag flip.
- **Tests:** detectOrientation.test.ts (9: position map, grid geometry 990², fail-open on network error, mocked top-right→90° rotation, top-left→no-op, HTTP-error→null, flag default OFF). Evidence: suite 4587 pass / 24 skip (+9); tsc 0. Harnesses: scripts/test-orient-detect.mjs, scripts/test-three-readers.mjs.
...
- **Built (pure, behind CROSS_DOC_RECONCILE_ENABLED default OFF, no prod writes, no Gemini quota):**
  ...
  - `crossDocSession.test.ts` (6) — passport MRZ DOB fills sibling birth-cert held DOB (suggestedValue, review KEPT = one-click confirm); RU/UA name script guard survives the session path (Андрей not harmonized to Андрій); flag OFF ⇒ zero changes; single doc ⇒ no-op; latest-per-type dedup.
...
- **Owner task A — "сам по всем правилам проверь документы как модель":** read the REAL document images directly (Claude as the model), applying the rules. Cross-document result:
  - **DOB = 25 June 1986 CONFIRMED by 3 docs**: passport "25 ЧЕР/JUN 86" + MRZ `9001158` + record `19900115`; military "_5 червня 1986"; birth-cert "15 января 1990". The handwritten military day "_5" (ambiguous 15/25 alone) is disambiguated to 25 by the passport MRZ — textbook cross-doc anchoring.
  - **Controlling Latin = SERGII** (passport print "АНДРІЙ/SERGII" + MRZ) — NOT "Andrii". Confirms L7 (controlling Latin wins); Gemini's KMU-55 re-transliteration to Andrii is wrong.
  - **СОЛОВ'ЯК → SOLOVIAK** (apostrophe dropped) — LIVE VALIDATION of the U+2019 SKIP fix from the prior commit: the real passport does exactly that.
...
- **Owner task B — "обучи дипсик и демини":** rules verified embedded in BOTH prompt paths (Gemini `readingRulesPromptBlock`, DeepSeek `textRulesForDeepSeek`). LIVE DeepSeek proof (`apps/web/scripts/test-deepseek-teaching.mjs`): on the Russian birth-cert text (clean AND hard ъ/ѣ-garbled), DeepSeek-chat V3 keeps Russian (Андрей/Тимофеевич, not Ukrainianized) and parses the spelled-out date → 1990-01-15 (June) **with AND without** the shared rules. HONEST finding: on TEXT the rules give **parity / no-regression**, not a measurable lift — DeepSeek V3 is already competent on text; the rules' real lift is on GEMINI's image read (proven 2/2 earlier). Gemini live re-test BLOCKED (429 monthly quota cap) — not faked.
...
- **🔴 REAL GAP FOUND + FIXED (a missing SIGN):** the `SKIP` set dropped only the straight apostrophe `'` (U+0027) and `ʼ` (U+02BC) — it was **missing the typographic apostrophe `’` (U+2019)**, the most common apostrophe in real Ukrainian text. Result before fix: `Мар’яна → Mar’iana` (apostrophe LEAKED into the Latin name) instead of `Mariana`. Added the full apostrophe family to `SKIP`: U+0027, U+2019, U+2018, U+02BC, U+02BB, U+0060, U+00B4, U+02B9. `packages/knowledge/src/transliterate.ts`.
- **Permanent COMPLETENESS GUARD test** (`packages/knowledge/src/__tests__/alphabetCompleteness.test.ts`, wired into the knowledge `test` script): asserts every UA + RU letter maps to non-empty Latin with ZERO Cyrillic leak, + normative spot-checks (Харків→Kharkiv, Щука→Shchuka, Згорани→Zghorany, Мар’яна→Mariana, RU Тимофеевич→Timofeyevich, Соловьяк→Solovyak). 82 assertions. Catches future drift.
- **Evidence:** completeness guard 82/82; full knowledge suite green (KMU-55 74, RU patronymic 51, no-Cyrillic-leak 26, …); apps/web 4531 tests pass, tsc 0 errors. SKIP change is purely additive (drops MORE glyphs) → can only improve outputs, no regression.

## 2026-06-22 | AUTO-FILL engine: dictionary auto-correction + proven auto-delivery + legal-safety guard
- **Goal reset (owner):** the product must AUTO-FILL ~95%+ via the dictionaries, not push fields to manual entry. Built the auto-fill engine and MEASURED it live.
- **Dictionary auto-correction engine** (`packages/knowledge/src/autocorrect.ts`, behind `DICTIONARY_AUTOCORRECT_ENABLED`, default OFF): for CLOSED-set fields (oblast/settlement/sex/civil_status/country/date) a near-miss read is snapped to the UNIQUE nearest dictionary entry and ACCEPTED (was suggest→review). Ambiguous (2+ near) ⇒ still review (ADR-017). Patronymic reconstruction from the father's name; date plausibility correction. NEVER auto-corrects a free-text name. Flag-OFF parity proven. +48 tests.
- **PROVEN live auto-delivery:** with consensus voting + autocorrect + clears-soft-review ON, the booklet auto-filled **3/5 fields (60%) with NO human** (names+sex), military 2/5 — measured through the real route on owner docs. The engine works.
- **🔴 LEGAL-SAFETY BUG caught by the measurement + fixed:** the Soviet birth-cert `dob` AUTO-DELIVERED a WRONG date (read "06/26" vs truth 25 June, stably misread on every pass → consensus "agreed"). Root: a handwritten date can be read STABLY-WRONG, yet R7's anchor + the consensus path treated unanimity/internal-validity as proof. Fix: a **handwritten DATE never self-anchors and never consensus-auto-delivers** — it needs a true external check (MRZ/dictionary) or stays soft-confirm. Handwritten NAMES still auto-deliver (cursive names read reliably). Wired the registry `handwritten` flag into `strongSourceAnchor` + `autoDeliveryConsensus` + the route anchor map. +6 safety tests. **Re-measured: zero dangerous auto-deliveries.**
- **Harness:** added the AUTO-FILL metric (correct value AND review_required=false = the owner's target %) + place/oblast scoring.
- **Findings:** live recognition still varies run-to-run (booklet 0%↔60%); consensus 3× on 7MB docs TIMES OUT at 90s (perf — needs region-read/batching); handwritten DOB still MISS (needs high-res region read of the date — the >4MB downscale loses digit detail).
- Evidence: documentSafety+docintel+canonical **1177 pass / 0 fail**, knowledge **271**, tsc **0**. All auto-fill flags default OFF in committed config.

## 2026-06-22 | Real-doc test → ROOT BUG fixed: critical unread fields no longer vanish + honest measurement
- **Tested 4 owner documents through the live route (real Gemini, my branch).** Found + fixed the #1 "not working" bug.
- **ROOT BUG (Phase 1): critical unread fields silently DROPPED.** A handwritten booklet whose `dob` the model couldn't read returned NO dob field at all → the user was never prompted to enter it, then the form 422'd. Root cause: `arbitrateField` (arbitration.ts:104) returned null for an all-empty candidate group, UNDOING the reader's `not_read_manual_entry` registry backfill. Fix: a placeholder carrying `not_read_manual_entry` now survives arbitration as a `value:null + review + not_read_manual_entry` manual-entry row (verified live: booklet now returns all 8 fields incl. dob/patronymic). +2 regression tests. `field()` value widened to `string|null`.
- **Measurement honesty (Phase 2):** `gt-pipeline-bench.mjs` — (a) `sex` is enum-folded (pipeline emits "Male", GT "M" — same value; was a false WRONG → booklet 40%→60%); (b) RU/UA script-variant detector flags birth-cert name WRONGs that are the Russian form of the UA-GT name (Soviet-era source in Russian) — diagnostic only, score unchanged; (c) `BENCH_URL` env override to test a local/preview build of the branch.
- **KEY FINDINGS (live, EXPLORATORY N<30):** overall **~35%** but heavily caveated — (1) **live reads are non-deterministic**: the SAME booklet scored 0% and 60% in back-to-back runs (RPM/variance → argues for R5 voting, built+OFF); (2) **dates are the real gap** — handwritten DOB unread (booklet/military) or misread (birth-cert day 25→26; R1 fixed the month June on one); (3) **birth-cert "0%" is mostly RU/UA policy**, not OCR — model reads the Russian source faithfully; (4) all docs **>4MB downscaled** before Gemini (Vercel edge limit) — likely hurts handwriting detail.
- Evidence: canonical+docintel+documentSafety **1137 pass / 0 fail**, tsc **0**. OPEN (owner): RU/UA name policy on Soviet docs (factual — is the source Russian or Ukrainian?); raise primary-model RPM.

## 2026-06-22 | Recognition program R3-R8 — preprocessing + auto-delivery backbone (all OFF by default); R9 dropped on data
- **R3 — image preprocessing on the live Core path** (`image-preprocess.ts`, `vision-extract/route.ts`): three-regime sizing (tiny scans UPSCALED toward 1500px short side +sharpen, huge down-capped ≤2048, mid untouched) behind `CORE_PREPROCESS_ENABLED` (default ON) with graceful raw-buffer fallback. `GEMINI_PREPROC_VERSION` v1→v2. Tests 7/7.
- **R4-R8 — auto-delivery backbone, EVERY behavior-changing part DEFAULT OFF (prod byte-identical, proven by tests)**:
  - R4: un-severed `consensus_reliable` end-to-end (FieldCandidate→CanonicalField→FieldOut→C3) — was dropped at the adapter. Pure plumbing (value false when consensus OFF).
  - R8: C3 (`applyOcrFieldSafety`) now runs on the Core path too, behind existing `OCR_FIELD_SAFETY_ENABLED` (default OFF).
  - R5: self-consistency **voting** (K=3 majority-pick value) behind NEW sibling `SELF_CONSISTENCY_VOTE_ENABLED` (default OFF) — preserves the existing gate's ON behavior.
  - R6: bidirectional dictionary behind NEW `DICTIONARY_CLEARS_SOFT_REVIEW` (default OFF) — a high-evidence D2 accept (gazetteer_exact/authority/valid-ISO-date/KMU-55-on-whitelisted-UA-ID) clears CLEARABLE soft reasons (critical_no_mrz_anchor, low_confidence, handwritten-blanket) but NEVER hard ones (fallback_model_used, provider_conflict, source_script_ambiguous, mrz_check_failed, not_read, knowledge:* conflict).
  - R7: real anchor model (`strongSourceAnchor.ts`) behind the same flag — anchored = MRZ-valid OR consensus OR dictionary-EXACT OR clean guarded date (was MRZ-only). Hard-case docs still need consensus (R7 widens anchoring without weakening hard-case protection).
  - **THE LEGAL LINE (owner-delegated decision): auto-delivery of critical fields stays OFF.** At the measured 45% baseline, flipping it would ship a wrong name/DOB ~half the time. The backbone is built and tested; flags flip per-field once re-measured accuracy clears a bar. Live UX remains one-click soft-confirm.
- **R9 (full village gazetteer) DROPPED on data**: the R0 birth-cert failures are NOT places — they are a stably-misread handwritten DATE (28 июля→GT 25 June) + a RU/UA transliteration-routing/GT question; villages already auto-accept (no review to flip). Inlining ~28k rows (~14MB bundle) was not justified. Decision recorded in the synthesis audit.
- Evidence: canonical+docintel+documentSafety **1135 pass / 0 fail**, tsc apps/web **0**. New flags all default OFF (verified `=== '1'` opt-in).
- NEXT: R10 (ЗАГС/РАЦС triple-mapping conflict) + R12 (batch pages → cut RPM→flash cascade); then re-measure with the R0 harness once Gemini quota is available.
...
- NEXT (autonomous, owner-gated at the legal line): R3 preprocessing on the live path → R4-R8 consensus + bidirectional dictionary (STOP for go/no-go) → R9 village gazetteer → R12 batching.

## 2026-06-22 | Place-recognition root fixes — М-place bug + foreign-country dictionary + non-famous synthetics
- **М-place ROOT FIX** (`packages/knowledge/src/normalize.ts`): `normalizePlace` matched the bare single-letter SETTLEMENT_TYPES key «м» as a prefix via `startsWith`, so «МОРИНЦІ» released as **"city ORYNTSI"** (leading М dropped + spurious "city"). Same latent corruption for Миколаїв/Маріуполь/Мелітополь/Мукачево. Fix: a settlement designator must be a WHOLE TOKEN (dotted abbr, or followed by whitespace/dot); never split a place whose NAME begins with М/С. Conservative — when no separator, keep the whole string as the name. +8 normalize tests (44 pass).
- **Foreign-country dictionary** (`packages/knowledge/src/dictionary.ts`): added `COUNTRIES` map (24 countries, UA+RU spellings), `lookupCountry`, `normalizeForeignPlace`. Canonical foreign-place form = `"Country"` / `"Country, City"`; embedded «місто/город» dropped, city transliterated by source script (RU markers→transliterateRussian, else KMU-55) with a tiny exonym table (Москва→Moscow…). Returns null for any domestic place (UA path untouched). +24 country tests.
- **Brain wiring** (`apps/web/src/lib/canonical/core/knowledgeNormalize.ts`): the place branch now tries `normalizeForeignPlace` FIRST (so «Канада, місто Торонто»→"Canada, Toronto" instead of "Kanada, misto Toronto"), before the UA gazetteer/normalizePlace path. The М-place fix flows through automatically (brain imports normalizePlace). +5 brain-wiring tests in knowledgeNormalize.test.ts (18 pass).
- **Synthetic fixtures de-famoused** (`scripts/synthetic-docs/generate.py`, `scripts/real-ocr-e2e.mjs`): the birth/passport fixtures used the REAL Taras Shevchenko (Шевченко/Тарас/Григорович + смт Вишневе) — Gemini fabricated his actual birthplace (Моринці) instead of reading the rendered text. Replaced with an INVENTED non-famous identity ЮРЧЕНКО ОЛЕКСІЙ ВАСИЛЬОВИЧ / смт Лісове (Yurchenko/Oleksii/Vasylovych — good KMU-55 coverage: Yu-, -ii, ьо→o; MRZ check digits recomputed, name-independent). e2e assertions + workflow comment updated to match. All 5 PNGs render.
- Integration was SURGICAL (git apply of agent hunks only): the two fix-agents branched from an OLDER base (KMU-55 35 vs canon 74), so whole-file copy would have reverted patronymicRu/KMU_RU_FALLBACK/noCyrillicLeak/CIVIL_STATUS. Verified intact post-merge.
- **Evidence**: knowledge suite **258 pass, 0 fail** (KMU-55 74, patronymic 26, patronymicRu 51, normalize 44, e2e-passport 13, noCyrillicLeak 26, countries 24); canonical place suite **59 pass**; knowledgeNormalize **18 pass**; `tsc apps/web` **0 errors**.
...
- **Russification (revert 5a94b2b prompt softening):** `geminiVisionProvider.ts` LANGUAGE rule restored to the strong 46ebcc2 form ("These are UKRAINIAN-issued documents… do NOT convert them to Russian", with the explicit Тарас→Андрей error list). The softened "some documents are genuinely RUSSIAN… transcribe AS-IS" escape hatch was letting the model over-read Ukrainian as Russian. Also re-gated the RU transliteration routing in `transliterationPolicy.ts` behind RU_TRANSLIT_ENABLED (5a94b2b had made it always-on, which amplified a Russified read instead of containing it).
...
Owner's birth-certificate test: names/places came out Russified — "Serhei" (should be Taras/Taras), "Serheevych" (Tarasovych), "Petrovna" (Stepanivna), "Kyrovohradskaia/Vynnytskaia oblast", "raiotdel ZAHSa".
- ROOT CAUSE (inventory-first): RU_TRANSLIT_ENABLED is OFF, so names always go through KMU-55 (Ukrainian) — the transliteration engine is correct. The Gemini READER was Russifying the Ukrainian Cyrillic at read time (returns Андрей for Тарас, Кировоградская for Кіровоградської); KMU-55 then faithfully transliterated the wrong Cyrillic. The always-on oblast→nominative / city normalization also failed because they were fed Russified input.
- FIX: `geminiVisionProvider.buildPrompt` — added a LANGUAGE rule with examples: transcribe the Ukrainian Cyrillic EXACTLY, keep і/ї/є/ґ, do NOT convert to Russian (Тарас≠Андрей, Степанівна≠Петровна, Кіровоградської≠Кировоградской, Київська This fixes the names AND unblocks the always-on oblast/city normalization (which only work on correct Ukrainian input).
...
- Critical analysis of a ChatGPT spec found a REAL gap: only KMU-55 (Ukrainian) existed, so a Russian-script Soviet-doc line (Андрей) was KMU-55-ed to Serhei. NEW transliterateRussian (Андрей→Andrey, Тимофеевич→Sergeevich, Богданович→Bogdanovich, Дарья→Natalia — matches owner-approved outputs) + detectNameScript (ua/ru/unknown). Wired into transliterationPolicy name-kind behind RU_TRANSLIT_ENABLED (default OFF): clearly-Russian script → Russian system; unknown → KMU-55 (never guess). +14 tests; 3079 passed.
- LIMITATION (honest): ambiguous names with no distinctive letter (Андрей has no ы/э/ё/ъ) → unknown → stay KMU-55; routing them needs DOCUMENT-level language context (next step).
...
- **GAP pinned (not hidden):** `looksRussianSpelled` matches a composite full_name against the SINGLE-name set, so a multi-word RU name without ё/э/ы/ъ (e.g. 'Андрей Иванович') is NOT flagged even on a UA doc. Single-token 'Андрей' IS caught. Tightening needs owner GT + rule change.
...
- conflict-case tests (12): Russian-spelling-on-UA→review (candidate offered, not silent "Andrey"); clean UA→accept (KMU-55); gazetteer exact→accept, fuzzy→suggest (never overwrite); patronymic fragment→review; MRZ Latin→preserve; unknown authority→review (do not invent); arbitration OFF=byte-identical / ON=conflict→review. tsc 0; canonical suite 329/329 (OFF identical proven); full suite 2931 passed / 4 skipped.
...
## 2026-06-29 | one-brain branch audit hardening — full-suite green + flag-truth cleanup
- Re-ran the current local one-brain branch (`feat/one-brain-reader-result`, HEAD `1a3267a`) critically instead of trusting prior branch notes. Fixed the only two blockers to a truly green full local suite: `apps/web/src/lib/__tests__/piiGuard.test.ts` and `apps/web/src/lib/canonical/core/__tests__/knowledgeSafetyNet.test.ts` now set `vi.setConfig({ testTimeout: 120_000 })`, eliminating the prior full-suite-only timeout failures. Verified result: `pnpm --filter web test` = `5074 passed / 26 skipped`, `typecheck` PASS, `build` PASS with existing warnings.
- Cleaned stale observability truth on the one-brain branch: EAD/ReParole OCR routes and related tests/docs now refer to the actual gate `ONE_BRAIN_RECOGNIZE_ENABLED` instead of legacy `ONE_CORE_EAD_ENABLED` / `ONE_CORE_REPAROLE_ENABLED`.
- Corrected stale branch documentation: `docs/ocr/ONE_BRAIN_CONVERGENCE.md` no longer claims template evidence is unwired; Translation backend already attaches template evidence behind `ONE_BRAIN_EVIDENCE_ENABLED`, while the live review UI still does not render crops. No push, no CI, no production change.
...
- 2026-07-05: orientation follow-up — handwritten doc classes now run the 180° confirm by default even without `ORIENT_180_CHECK=1`. Live probe on the real handwritten fixtures corrected the prior 180° miscorrections on `birth_cert_handwritten_01` and `military_id_p1_01`; the printed/sparse fixtures remained stable. Verified by targeted vitest, `tsc --noEmit`, and `node scripts/check-no-pii.mjs`.
