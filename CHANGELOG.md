...
**#10 prose translator wired:** vision-extract now passes a DeepSeek proseTranslator (names/numbers LOCKED) into analyze → free text the glossary did not cover (e.g. a registry office full name) is translated instead of dropped. Fails open (keeps original + review). Uses DEEPSEEK_API_KEY.
...
**CI fix:** reworded a #16 comment that tripped content-guard Rule 4 ("certified translation" product claim) → "translation draft must be signed". Content guards 0 violations.
...
> «Коммит 3580315 («Pure CSS change, no JSX touched») — это ложь. На деле 354 файла / +101k строк, и он впервые залил в git 34 дампа форм I-821/I-765 с реальным PII владельца ((Last Name) REDACTED). Retention-policy ловила .zip/.pdf/.png, но .txt-дампы просочились.»
...
- **17 test files** under `apps/web/src/lib/**/__tests__/` and `apps/web/tests/e2e/` — they use «REDACTED» / «Тростянець» / «Sergii» as deterministic KMU-55 transliteration fixtures. Removing them = breaking the entire OCR-stability test suite. The owner's name in these files is a documented, intentional regression fixture.
- **`apps/web/src/lib/tps/ocr/postExtractNormalize.ts`** — handles «REDACTED» as a known-good normalization synonym (vision-arbiter-proof N=1 reference). Intentional.
...
2. **Markdown narrative still names «REDACTED / Тростянець / Sergii» in prose.** 10 markdown + 4 YAML files in `docs/audit/`, `docs/reports/`, `docs/architecture/`, `STATUS.md`, `HANDOFF.md` and this CHANGELOG. Can be redacted in a follow-up; current decision is to keep historical narrative intact for engineering memory.
...
- `git grep -l REDACTED` after stage: 34 (down from 160) — all are tests / intentional code / narrative docs
...
- **Live proof (Gemini 2.5 Flash, owner's booklet):** all 5 identity fields read correctly in Cyrillic — REDACTED_NAME / Сергій / Сергійович / 25 червня 1986 / Тростянець. Fixes the two production failures (patronymic "Yovych", city "Prostianets") and recovers given_name. 6.85s, ~0.12¢/doc.
