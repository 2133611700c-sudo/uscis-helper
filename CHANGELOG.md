...
<!-- 2026-07-25 Resend recovery: historical job 89509758114 proves the exact failure is "API key is invalid" (HTTP 400 validation_error). Add PII-safe allowlisted provider diagnostics and an HMAC-authenticated /api/internal/monitor-digest production relay so GitHub can use Vercel's existing Resend configuration without copying or exposing the key. Relay enforces five-minute signatures, signature-derived Resend idempotency, 200KB payload limit, validated recipient/subject/html, bounded provider timeout, no-store responses, and safe logs only. Temporary diagnostic push trigger removed; workflow cutover remains a separate post-deployment proof. -->
