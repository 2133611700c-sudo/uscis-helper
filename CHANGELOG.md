...
<!-- 2026-07-25 Resend diagnostic (in flight): runs 30162581912 and 30162668996 identified validation_error HTTP 400 but no safe hint; Resend may return validation text under `error` instead of `message`. Handle both response shapes and still emit only allowlisted policy hints (testing-recipient restriction, domain verification, quota, or field validation). Provider messages, addresses, digest content, and secrets remain excluded. Temporary branch push trigger remains only for the final diagnostic run. -->
