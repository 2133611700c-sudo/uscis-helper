...
<!-- 2026-07-25 Resend relay cutover (in flight): after PR #33 merged the authenticated production relay, change Federal Register digest delivery from the conclusively invalid GitHub RESEND_API_KEY (job 89509758114) to a signed relay request. GitHub uses the existing Supabase service-role secret only as HMAC input; the raw secret is never transmitted and Vercel keeps ownership of RESEND_API_KEY/RESEND_FROM. Temporary branch push trigger exists solely for one live delivery proof and must be removed before merge. -->
