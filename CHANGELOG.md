...
## 2026-06-03 — DOCUMENT_UNDERSTANDING_GAP_CLOSURE: military ID + birth certificate schemas

New extraction modules close the gap where docHint=military_id and docHint=birth_certificate returned 0 structured fields despite OCR text being available. `militaryId.ts`: regex + keyword anchors extract family_name/given_name/patronymic/dob/serial_number/issuing_authority from raw OCR. Ukrainian month name parser covers all 12 months. review_required=true on every field always (hard-case policy). `birthCertificate.ts`: role-grounded extraction separates child/parent/registration blocks by "Батько"/"Мати" header anchors. child_* fields never populated with parent names. wrong_person_risk=true when structure unclear. review_required=true always (hard-case class — benchmark proved wrong-person risk). Both modules wired into `/api/tps/ocr/extract` switch + documentContracts.ts slots added. documentClassPolicy.ts updated to handle military_id + birth_certificate hints. Forbidden fields enforced: I-94/A-number/EAD/address never in output. 22 new unit tests (11 per module). Tests: 2717/2717. tsc: 0. Gazetteer BLOCKED_NO_SOURCE (no KOATUU data available). Model routing clean (gemini-2.0-flash not in any runtime path). Branch: feat/document-understanding-schemas.
...
## 2026-06-03 — MRZ_AUTHORITY_WIRED_CODE_READY: mrzCandidatesFromText connected to TPS + Re-Parole Core routes

Wire `mrzCandidatesFromText` into TPS (`/api/tps/ocr/extract`, ONE_CORE_TPS_ENABLED path) and Re-Parole (`/api/reparole/ocr/extract`) for `ua_international_passport`. TPS uses Google Vision `result.raw_text` already in-route. Re-Parole runs Vision OCR in parallel with Gemini docintel via `Promise.all`. MRZ candidates injected before `arbitrateDocument` in both routes. Guard: `docintelId === 'ua_international_passport'` — EAD/booklet/i94/dl not affected. Forbidden fields (i94, a_number, ead_category, us_address, eligibility, patronymic) never emitted. New test file: `canonical/core/__tests__/mrzWiringProof.test.ts` — 18 arbitration-level proof tests covering: valid MRZ wins over Gemini for all 7 fields, invalid MRZ forces review_required=true (not silent), missing MRZ falls through with critical_no_mrz_anchor, forbidden fields never in output, EAD not affected, conflict evidence preserved. Tests: 2664/2664. tsc: 0. Branch: feat/mrz-passport-authority, PR #74.

## 2026-06-03 — MRZ international passport authority for Document Core

New file: `canonical/core/mrzAuthority.ts`. Wraps `parseMrz` (from `@uscis-helper/knowledge`) into a `FieldCandidate[]` producer implementing the `CoreReaders.mrzRead` interface. Valid MRZ (all ICAO check digits pass) → 7 candidates with `source='mrz'`, `mrzCheckValid=true`, `confidence=0.99`, `reviewRequired=false` — Core arbitration gives these absolute authority over `PASSPORT_MRZ_FIELDS`. Invalid MRZ → candidates with `mrzCheckValid=false`, `confidence=0.3`, `reviewRequired=true`, `reviewReasons=['mrz_check_failed']` — NOT silent fallback. Missing MRZ → empty array, Core falls through to visual candidates. Exports: `mrzCandidatesFromText` (sync), `mrzReadFromOcrText` (async CoreReaders.mrzRead compatible), `MRZ_CONTROLLED_FIELDS` (7 fields), `MRZ_FORBIDDEN_FIELDS` (10 fields that MRZ must never populate). New test file: `canonical/core/__tests__/mrzAuthority.test.ts` — 36 tests: all 7 controlled fields on valid MRZ, invalid/missing MRZ, sex edge cases, 10 forbidden fields hard-checked, MRZ_CONTROLLED_FIELDS in PASSPORT_MRZ_FIELDS verified, async interface, invented_fields_count=0. No existing code changed. Full suite: 2646/2646. tsc: 0. Branch: feat/mrz-passport-authority.

## 2026-06-03 — B4 UI WIRING: EAD wizard calls /api/ead/ocr/extract when flag ON

B4 UI wiring complete. `EADWizard.tsx` now has an optional upload prefill step behind `NEXT_PUBLIC_ONE_CORE_EAD_ENABLED=true` flag. When flag OFF (default): wizard is unchanged — old 7-step manual form, 0 behavior change. When flag ON: a new `StepUpload` component appears at step index 2 (after category selection, before personal info). User selects document type (passport / EAD card / I-94), uploads an image, Core extracts fields, and the form is prefilled. Prefill mapping: `family_name`→`lastName`, `given_name`→`firstName`, `date_of_birth`→`dob`, `sex`→`gender` (M/F), `country_of_birth`→`countryOfBirth`, `a_number`→`alienNumber` (source-gated by adapter; null if source not EAD/I-797). Skip button always visible — upload step never blocks. Review warning shown when `review_required=true` from Core. New test file: `components/services/ead/__tests__/eadWizardUiWiring.test.ts` — 45 tests covering flag wiring, route reference, docHints, prefill mapping, source gates (passport→a_number=null, ead_category=null, i94=null), review_required propagation, architecture contract markers, invented_fields_count=0 all cases. Tests: 2610/2610. tsc: 0. Branch: feat/b4-ead-core. Status: ONE_BRAIN_COMPLETE_CODE_READY — ready to merge PR #73 + enable flags + redeploy.

## 2026-06-03 — B4: EAD consumes CanonicalDocumentResult (ONE_BRAIN_COMPLETE_CODE_READY)

B4 wires EAD to the same Document Core as TPS (B1), Translation (B2), and Re-Parole (B3). ONE_BRAIN_COMPLETE_CODE_READY: all 4 products have Core adapters. New files: (1) `canonical/core/eadAdapter.ts` — pure `toEadAnswers()` adapter; source-gated field mapping: EAD/USCIS fields (a_number, ead_category, card_number, ead_validity_*) null unless source is EAD card / I-766 / I-797; I-94 fields null unless source is I-94; us_address null unless source is DL/state_id (never from passport); invented_fields_count always 0 (type-level `0` literal); (2) `app/api/ead/ocr/extract/route.ts` — new EAD Core OCR route behind ONE_CORE_EAD_ENABLED=true flag (default: false); returns EadCoreAnswers JSON; (3) `canonical/core/__tests__/eadAdapter.test.ts` — 74 adapter tests: passport-only proof (a_number=null, category=null, i94=null, address=null), EAD/I-94/DL source gates, purity. Tests: 74 new + 2565/2565 full suite. tsc: 0 errors. Branch: feat/b4-ead-core.

## 2026-06-03 — B3 UI WIRING: Re-Parole wizard calls /api/reparole/ocr/extract when flag ON

B3 UI wiring complete. `ReparoleWizardV2.tsx` now routes OCR calls based on `NEXT_PUBLIC_ONE_CORE_REPAROLE_ENABLED` env var. When flag OFF (default): wizard calls `/api/tps/ocr/extract` — old behavior 100% unchanged. When flag ON and slot is `passport` or `booklet`: wizard calls `/api/reparole/ocr/extract` (Core path). US-form slots (i94, ead, dl) always use old path regardless of flag — Core doesn't cover them yet. Core response shape (`ReParoleCoreAnswers`) mapped to wizard `FieldExtraction` records: top-level fields parsed, `date_of_birth` aliased to `dob`, `requires_review` propagated from `review_required` flag and `uncertain_fields` set, source set to `ai_brain`. I-94 fields: null when no I-94 source (not invented). New test file: `app/api/reparole/ocr/extract/__tests__/uiWiring.test.ts` — 12 tests: source-level wiring guards (flag constant, CORE_COVERED_SLOTS, route selection logic, response shape branching) + functional response parsing tests (Core shape, fallback_used, i94 non-invention). Tests: 12 new + 2503/2503 full suite. tsc: 0 errors. Branch: feat/b3-reparole-core. Status: ONE_BRAIN_PARTIAL_3_PRODUCTS when owner sets flag ON in Vercel.

## 2026-06-03 — B3: Re-Parole consumes CanonicalDocumentResult (ONE_BRAIN_PARTIAL_3_PRODUCTS)

B3 wires Re-Parole to the same Document Core as TPS (B1) and Translation (B2). New files: (1) canonical/core/reParoleAdapter.ts — pure toReParoleCoreAnswers() adapter: no OCR, no Gemini, no I/O; maps CanonicalField[] → ReParoleCoreAnswers by key lookup with alias support (dob, nationality, place_of_birth, date_of_expiry, etc.); I-94 fields stay null when absent (no invention); review_required never lowered; uncertain_fields tracked; core_status=ok/partial/failed. (2) app/api/reparole/ocr/extract/route.ts — new dedicated Re-Parole OCR route behind ONE_CORE_REPAROLE_ENABLED=true flag (default: false); docintel → arbitrateDocument → toReParoleCoreAnswers → JSON; old /api/tps/ocr/extract path unchanged. (3) canonical/core/__tests__/reParoleAdapter.test.ts — 29 tests covering identity mapping, I-94 non-invention, review_required propagation, uncertain_fields, core_status, adapter purity. Flag OFF by default. Branch: feat/b3-reparole-core. Tests: 29/29 new + 2491/2491 full suite. tsc: 0 errors.

## 2026-06-03 — Document-class policy WIRED into live extraction path (POLICY_WIRED)

Wired 3 policy guards into live extraction routes (branch feat/wire-document-class-policy). Routes changed: `tps/ocr/extract/route.ts` + `translation/vision-extract/route.ts`. Added mapping helpers to documentClassPolicy.ts: `docintelIdToDocumentClass()`, `tpsHintToDocumentClass()`, `isUkrainianIdentityDoc()`. Guard 1: `checkImageQuality()` called before OCR/Gemini call — blocks tiny images (needs_better_scan response at status:200), warns on >2MB. Guard 2: `applyHardCaseReviewOverride()` called after extraction — forces review_required=true on hard-case classes (birth_certificate_handwritten, birth_certificate_soviet_bilingual, marriage_apostille, unknown_document) regardless of model output. Guard 3: `applyCertificateRoleGuard()` called after extraction — rejects generic family_name without role grounding on certificate docs; forces review_required=true on affected fields. US-form slots (i94/ead/dl/i797/tps_notice) excluded from policy via isUkrainianIdentityDoc() check. Tests: +24 new wiring invariant tests. Full suite 2462 pass / 0 fail. tsc 0. PR created on feat/wire-document-class-policy.
...
## 2026-06-02 — Session 92: Core field-vocab fixes + review carry-through + Translation wiring (branch feat/core-wire-translation)

Fixed three real-data bugs found on actual Ukrainian documents (Session 91 diagnosis). (1) `criticalityOf('dob')` returned 'low' — Gemini docintel emits `dob` key, Core mis-classified date of birth as low-criticality and auto-filled without review. Fixed: `dob` added as critical alias in policy.ts; birth-cert child fields (`child_family_name`, `child_given_name`, `child_patronymic`, `child_dob`, `child_date_of_birth`) added as critical. (2) reader `review_required` was silently dropped — `FieldCandidate` had no `reviewRequired` field, so blurry/uncertain reader flags were discarded by the Core. Fixed: `reviewRequired`/`reviewReasons` added to `FieldCandidate` type; `arbitrateField` carries the signal forward with `reader_review_required` reason. (3) Core was NOT wired to any product. Added `translationAdapter.ts` (docintelToCandidate / canonicalToFieldOut) and `ONE_BRAIN_CORE_ENABLED=1` path in vision-extract/route.ts — first live wiring of the Core arbitration; flag OFF = byte-for-byte identical to before. Evidence: coreFixes.test.ts 12/12; full web 2389 pass; tsc 0. Files: policy.ts, core/types.ts, core/arbitration.ts, core/translationAdapter.ts (new), vision-extract/route.ts, core/__tests__/coreFixes.test.ts (new).

## 2026-05-31 — Session 91: PROD HOTFIX — Gemini key name mismatch (branch fix/gemini-key-066-name)

Found + verified the root cause of "ничего не распознаётся": prod /api/translation/vision-extract returns 502 on real docs because the code reads the Gemini key only from GEMINI_API_KEY_PAY/GEMINI_API_KEY, while the owner uploaded the working key to Vercel under name GEMINI_API_KEY_066 → app uses the dead key → central-brain reads 0 fields → route returns 502 by design (route.ts:130). Fix: new lib/gemini/apiKey.ts getGeminiApiKey() reads the key from ANY GEMINI_API_KEY* env name (owner kept renaming: 066 → GEMINI_API_KEY2; suffixed preferred over bare), wired into both call sites; apiKey.test 6/6; verified end-to-end that var named GEMINI_API_KEY2 makes readDocument read the real booklet (ok=true). tsc 0; full web 2383 pass; guard 0. Proven: with the working key, single Gemini read reads both owner docs correctly (booklet 25s, birth cert 8.6s) where central-brain consensus returns 0→502 — validates the one-brain single-read approach. Two real-data Core bugs found (key-vocabulary mismatch; review_required not carried) to fix next. Also fixed route.ts: central-brain reading 0 fields now DEGRADES to the legacy single-read path (proven) instead of returning 502. PR #67 merged. Files: api/translation/vision-extract/route.ts, lib/docintel/providers/geminiVisionProvider.ts, lib/gemini/apiKey.ts.

## 2026-05-30 — Session 89: Reader-benchmark harness (branch feat/reader-benchmark)

Merged #64 (one-brain v1 spine foundation). Built the reader-benchmark: compare old TPS / old Translation(docintel) / MRZ / new Core vs hand-filled ground truth, metric critical_wrong_count (coverage secondary). Built on VERIFIED real signatures (readDocument, parseTd3+checkResults, preprocessImage — 3 parallel Explore agents). New `apps/web/src/lib/canonical/core/benchmark/`: passportTruth.ts (owner flat schema latin+cyrillic + criticality), mappers.ts (mapMrz/mapTranslation/mapTps/mapCore → common ProducedField[]; MRZ date→ISO + per-field check-digit→review; MRZ has no cyrillic/patronymic/place by design), runReaderBenchmark.ts (+ PII-free summarizeBenchmark). Updated groundTruth.example.json to owner flat schema. New benchmark.test.ts 7/7; full web 2377 pass; tsc 0; content-guard 0. NOT LIVE: scores PROVIDED outputs, no live engine call, no product migrated, no flags. Needs owner real doc + ground truth + a live runner. Migration = manual approval. Files: core/benchmark/{passportTruth,mappers,runReaderBenchmark}.ts, core/groundTruth.example.json, core/__tests__/benchmark.test.ts, HANDOFF/STATUS/CHANGELOG.
...
**Live E2E + settlement bug:** added a GATED live integrated-pipeline test (pipeline.live.e2e.test.ts, LIVE_E2E=1) — ran the REAL preprocess→3.1-pro→GoogleVision→registry chain on the real military ID. It caught a real bug units missed: when recognition dumps the whole place line ("смт Тростянець Вінницької обл.") into one field, lookupSettlement failed → lost the type + leaked the oblast. Fixed: lookupSettlement now tries progressively shorter leading word-groups down to the city token. Now: Kuropiatnyk + "Trostianets (urban-type settlement)" end-to-end. +offline guard test.

**Live E2E extended (3 docs):** pipeline.live.e2e now covers military + international passport (MRZ controlling-Latin: KUROPIATNYK/FU262473) + handwritten 1986 birth cert. All PASS live — no new bugs. Gated (LIVE_E2E=1), skips in CI.
...
> Â«ÐÐ¾Ð¼Ð¼Ð¸Ñ 3580315 (Â«Pure CSS change, no JSX touchedÂ») â ÑÑÐ¾ Ð»Ð¾Ð¶Ñ. ÐÐ° Ð´ÐµÐ»Ðµ 354 ÑÐ°Ð¹Ð»Ð° / +101k ÑÑÑÐ¾Ðº, Ð¸ Ð¾Ð½ Ð²Ð¿ÐµÑÐ²ÑÐµ Ð·Ð°Ð»Ð¸Ð» Ð² git 34 Ð´Ð°Ð¼Ð¿Ð° ÑÐ¾ÑÐ¼ I-821/I-765 Ñ ÑÐµÐ°Ð»ÑÐ½ÑÐ¼ PII Ð²Ð»Ð°Ð´ÐµÐ»ÑÑÐ° ((Last Name) Kuropiatnyk). Retention-policy Ð»Ð¾Ð²Ð¸Ð»Ð° .zip/.pdf/.png, Ð½Ð¾ .txt-Ð´Ð°Ð¼Ð¿Ñ Ð¿ÑÐ¾ÑÐ¾ÑÐ¸Ð»Ð¸ÑÑ.Â»
...
- **17 test files** under `apps/web/src/lib/**/__tests__/` and `apps/web/tests/e2e/` â they use Â«KuropiatnykÂ» / Â«Ð¢ÑÐ¾ÑÑÑÐ½ÐµÑÑÂ» / Â«SergiiÂ» as deterministic KMU-55 transliteration fixtures. Removing them = breaking the entire OCR-stability test suite. The owner's name in these files is a documented, intentional regression fixture.
- **`apps/web/src/lib/tps/ocr/postExtractNormalize.ts`** â handles Â«KuropiatnykÂ» as a known-good normalization synonym (vision-arbiter-proof N=1 reference). Intentional.
...
2. **Markdown narrative still names Â«Kuropiatnyk / Ð¢ÑÐ¾ÑÑÑÐ½ÐµÑÑ / SergiiÂ» in prose.** 10 markdown + 4 YAML files in `docs/audit/`, `docs/reports/`, `docs/architecture/`, `STATUS.md`, `HANDOFF.md` and this CHANGELOG. Can be redacted in a follow-up; current decision is to keep historical narrative intact for engineering memory.
...
- `git grep -l Kuropiatnyk` after stage: 34 (down from 160) â all are tests / intentional code / narrative docs
...
- **Tests**: unit (exact KMU-55: Kuropiatnyk/Serhii/Serhiiovych/Trostianets) + live integration (self-skips in CI; RUN_LIVE_VISION=1 to run). LIVE on owner booklet through production code reproduced correct output, fixing prod Yovych/Prostianets. 2115 pass + 1 skip, 0 type errors, drift gate green.
...
- Cyrillic input (booklet) â `transliterateKMU55()` â e.g. "ÐÑÑÐ¾Ð¿'ÑÑÐ½Ð¸Ðº" â "Kuropiatnyk"
- Latin input (passport MRZ / EAD / I-94) â passthrough with garbage guard
- ALL-CAPS Latin input title-cased ("KUROPIATNYK" â "Kuropiatnyk")
