...
## 2026-06-20 | Golden vectors — deterministic proof the central brain + dictionaries are CORRECT (NO Gemini/OCR), 2 real wiring bugs found
- GOAL (agent GOLDEN, branch `feat/tv2-golden-cyrillic-vectors` ← `feat/translation-v2-rebuild`): the #1 professional proof that "central brain + dictionaries + transliteration" produce the CORRECT output for known Ukrainian/Russian Cyrillic inputs, WITHOUT any external service. Pure-function golden vectors in, asserted bytes out. Two NEW test files ONLY — no source/dictionary/brain file was edited (correctness bugs were REPORTED, not fixed, per task scope).
- NEW `packages/knowledge/src/__tests__/goldenDictionaryVectors.test.ts` (59 asserts, plain `tsx` runner, exits 1 on any failure → cannot fake a green). Covers: KMU-55 (Шевченко→Shevchenko, Ющенко→Yushchenko, Їжакевич→Yizhakevych, Згурський→Zghurskyi ЗГ-special, ALL-CAPS→ALL-CAPS, Київ→Kyiv #CorrectUA); Russian BGN/PCGN as-written (Иван→Ivan, Иванович→Ivanovich, Петрович→Petrovich, Эдуард→Eduard) + a contrast proof that KMU-55 on Russian gives the WRONG form; `detectNameScript` (Russian-only→'ru', Ukrainian-only→'ua', shared-letters→'unknown' i.e. NOT silent KMU-55); patronymic (valid read kept, bare "ович" fragment REJECTED, fragment reconstructed from given+sex, label="Patronymic" with "Middle Name" blocked); Міліція→Militsiya (never Police/Militia), УМВС→Regional Department of MIA; смт→"urban-type settlement" (never city/town); oblast genitive (Вінницької)→nominative "Vinnytsia Oblast"; gazetteer exact vs fuzzy-review; controlling MRZ Latin preserved + conflict→review; UA month-name + dot dates→USCIS.
- NEW `apps/web/src/lib/canonical/__tests__/cyrillicGoldenVectors.test.ts` (22 vitest tests). Same proofs driven through the CENTRAL BRAIN entry point `normalizeCanonicalValue` (canonical/core/knowledgeNormalize) — the layer the arbiter calls — asserting the full `KnowledgeDecision` (action + finalValue/candidateValue + provenance).
- TWO REAL VIOLATIONS FOUND (asserted as the ACTUAL engine output, flagged `// RULE VIOLATION`, NOT a faked pass) — both are BRAIN-WIRING bugs; the standalone dictionary layer is CORRECT (proven by the `place_settlement` reference vector + the knowledge suite):
  - **V1 — «смт» designator dropped.** For a `place_of_birth` / `*city*` Cyrillic value the brain calls `snapCity()` FIRST (`knowledgeNormalize.ts:201-203`). snapCity strips the «смт» prefix and returns an EXACT gazetteer match on the bare city ("Вишневе"), which the brain accepts as "Vyshneve" — the "urban-type settlement" designator is LOST. The correct `normalizePlace` fallback ("urban-type settlement Vyshneve") is never reached. Violates the HARD RULE "«смт» = urban-type settlement, NEVER city/town".
  - **V2 — issue-date misrouted to authority.** A field named `date_of_issue` / `issue_date` contains the substring "issu"; the brain's authority branch (`key_.includes('issu')`, `:216`) is tested BEFORE the date branch (`key_.includes('date')`, `:232`), so a valid Ukrainian date is sent to the authority handler → `authority.unknown` → review (finalValue=null) instead of `accept` "02/19/2003". `date_of_birth` (no "issu") works correctly.
- VERIFY: `npx tsc --noEmit -p apps/web/tsconfig.json` → 0 errors; knowledge `tsconfig` tsc → 0. knowledge golden vectors 59/59 pass (tsx). FULL web suite `pnpm --filter web exec vitest run` → **4293 passed / 24 skipped / 0 failed** (incl. the 22 new vitest tests); a single PRE-EXISTING intermittent flake exists in the base suite (unrelated to these deterministic vectors, which pass on every run). Files added: the two test files + STATUS/HANDOFF/CHANGELOG. #195.
