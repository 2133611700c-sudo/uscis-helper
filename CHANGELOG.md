...
## 2026-06-20 | Brain single-arbiter invariant — test-only proof (Translation path)
- Added `apps/web/src/lib/canonical/core/__tests__/brainSingleArbiterInvariant.test.ts` (NEW, test-only; no source touched). Proves the #1 integrity guarantee: NO field value reaches the translation output (rows/PDF) without passing the Central Brain (`arbitrateDocument`) → `buildCanonicalResult` (single envelope) → `getCanonicalValue` (C3 `finalValue===null` ⇒ null, no resurrection).
- 12 tests, 5 invariants: (1) C3 contract — `getCanonicalValue` returns null for `finalValue===null` even with normalizedValue/rawValue present; (2) a C3-rejected critical field → `toTranslationRows` emits `value=null`, never the raw guess, review state preserved; (3) valid MRZ (controlling Latin) wins over a re-transliteration candidate and survives to the row; invalid MRZ → review; (4) `buildCanonicalResult` is a PURE wrapper (fields passed by reference, deep-equal unchanged, only documented envelope fields derived); (5) SOURCE-INVARIANT (à la actions.security.test.ts) — `vision-extract/route.ts` builds rows ONLY via `toTranslationRows` over `*canonicalResult.fields`, never calls `canonicalToFieldOut` directly, never re-implements the `finalValue ?? normalizedValue` precedence, and the post-canonical role guard reads already-resolved `row.value`.
- VALUE-FLOW MAP (route.ts): Core path readDocument→buildCyrillicMap→docintelToCandidate→applyKnowledgeBrainIfEnabled(arbitrateDocument):366→buildCanonicalResult:381→toTranslationRows(canonicalResult.fields):413. Legacy path mirrors it: docintelToCandidate:519→applyKnowledgeBrainIfEnabled:535→buildCanonicalResult:541→toTranslationRows(legacyCanonicalResult.fields):578. Post-row guards (date ensemble, doc-class role guard, applyOcrFieldSafety) only re-map review flags over existing rows.
- NO BYPASS FOUND. Every value path to a translation row passes the brain+canonical accessor.
- Evidence: new file 12/12 green; FULL suite passed / 0 failed; `tsc --noEmit -p apps/web` = 0 errors.
...
## 2026-06-20 | Translation V2 — 5-agent forensic audit + P0-1 live email-relay fix (merged via #196 on main)
...
- FIXED (P0-1): `/api/translation/email` was an unauthenticated open email relay. Added `rateLimit('translation-email:'+ip, 5/hour)` → 429, with regression test. (Landed on main via #196; recorded here for continuity.)
