...
- **END-TO-END LIVE PROOF** (`scripts/test-wired-recover.mjs`, flag ON, real readDocument): birth cert recovered **7/7** empty fields, all matching GT — father "Куропятник Сергей Леонидович", mother "Куропятник Наталья Степановна", series "III-АМ № 428069", office "Тростянецкий райотдел ЗАГСа", + act/date/issue. All held for review, raw_cyrillic only (C3/transliteration own the canonical value), tagged `hires_tile_recovered`.
...
- **🔴 Root cause CONFIRMED = resolution, not field-spec/prompt:** at full-page scale (preprocess downscales long side to 2048) a dense handwritten region is ~4-5 px/letter → Gemini honestly returns can_read=false. A HIGH-RES crop (~2×) of the right page READ them — proven live: father "Куропятник Сергей Леонидович", mother "Куропятник Наталья Степановна", series "III-АМ № 428069", office "Тростянецкий райотдел ЗАГСа". Two things are BOTH necessary: hi-res crop AND a targeted aggressive prompt ("read these named fields, never empty for a present field") — the standard full-page prompt on a tile stays conservative.
