...
## 2026-06-20 | Translation V2 — FIXTURE-E2E: full pipeline proven without Gemini (synthetic OCR → real chain → poppler)
- PROBLEM: the live translation E2E is blocked on Gemini quota (429), so the FULL post-read pipeline had no end-to-end proof at prod defaults.
- FIX (test-only, NO product change): NEW `apps/web/src/app/api/translation/__tests__/translationPipelineFixtureE2E.test.ts` injects a SYNTHETIC, PII-free "OCR read" (`ExtractedDocField[]` — the exact shape `readDocument` emits) for a handwritten Ukrainian birth certificate and drives it through the SAME real post-read chain the route runs, in the SAME order: `buildCyrillicMap` → `docintelToCandidate` → `applyKnowledgeBrainIfEnabled` (REAL `arbitrateDocument`) → `buildCanonicalResult` → `toTranslationRows` → `applyOcrFieldSafety` (flow `translation_public`, the route's C3 critical-null guard) → `generateTranslationPDF` → poppler (`pdfinfo`/`pdftoppm`/`pdftotext`). The BRAIN (arbitration + C3 gate) is REAL, never mocked; only the camera/Gemini read is faked (the part blocked on quota). KMU-55 is REAL too — the fixture's Latin `value` is produced by the actual docintel `toCanonicalValue` (→ `transliterateKMU55`).
- FIXTURE includes the two hard cases: an AMBIGUOUS-SCRIPT name ("Петрова", no UA/RU-distinctive letter → `source_script_ambiguous`) and a LOW-CONFIDENCE critical field (`dob` conf 0.40 < 0.70 floor).
- CANONICAL-ROWS ASSERTS (always run): KMU-55 correct (Бондаренко→Bondarenko, Олександр→Oleksandr, смт Тростянець→"urban-type settlement Trostianets"); every handwritten field `review_required`; ambiguous name carries `source_script_ambiguous`; low-confidence critical `dob` → `value=null` + `finalValue=null` + `review_required` + raw parked in `candidate_value` (never guessed), `anyUnresolvedCritical=true`; all critical identity fields nulled candidate-only on the hard-case cert; a non-critical date survives C3 normalized to USCIS US format (04/15/1989).
- POPPLER (ran locally, pdfinfo 26.04.0; self-skips where absent): pages>=1, every page non-blank, English value present, "competent to translate" + "8 CFR" cert block + signer, and ZERO U+0400–U+04FF Cyrillic leak in the certified output.
- FILE OWNERSHIP: only the new test + these 3 guard docs. NO product/route/brain/dictionary edit; NO export seam needed (every pipeline function was already exported).
- VERIFY: tsc 0; new test 8/8 (full chain w/ poppler); FULL suite 4225 pass / 24 skip / 0 fail. #195.
