...
- **WEBHOOK BOUNDARY TESTS** `apps/web/src/app/api/stripe/webhook/__tests__/webhookPaymentBoundary.test.ts` (8 behavioral, mocks the Stripe SDK `constructEvent` boundary in test process only — NO real keys/bypass): missing-signature→400 (constructEvent never called), missing-secret→400 fail-closed, invalid-signature (SDK throws)→400 + zero DB writes, wrong event type→no transition, `payment_intent.payment_failed`→never marks paid/emailed, verified `checkout.session.completed`(translation)→audit_log + legacy update GATED on status='signed' AND email match (never an arbitrary order), no-customer-email→no order update, wrong-product(tps)→no translation/wizard update.
- **OPERATOR PIPELINE BEHAVIORAL** `apps/web/src/lib/translation/orders/__tests__/operatorPipelineBehavioral.test.ts` (29) — drives the REAL `lib/translation/orders`, `[id]/v2Actions.ts`, and `api/internal/translation-delivery` worker against a faithful in-process fake of the Phase 2 Supabase surface (`__tests__/fakeOrdersDb.ts` — mirrors `transition_translation_order`/`create_artifact_and_enqueue`/`claim_outbox_event` + the BEFORE-UPDATE triggers EXACTLY, including error messages so `classifyOrderError` routes identically). Synthetic verified-payment seeded INSIDE the test (post-`verifyStripeSessionPaid`). Covers Wave 3A (order exactly-once + idempotent replay, recipient from server only, canonical immutable, version+audit, actor-required, atomic artifact+outbox, INV-11 C3-null omit, duplicate-idempotency-key no 2nd outbox) + Wave 3B (operator authz 401/403/409, invalid transition blocked, override provenance=operator_override + base preserved + empty=null-reject, changeRecipient audited hashes-only; delivery exactly-once + hash-verify + NO re-render + idempotency-key flow + concurrent-claim one-winner + tamper→retry + transport-fail→retryable + crash-no-duplicate + missing-recipient→permanent) + CHAOS (DB-fail-between-artifact-and-outbox→full rollback no orphan, storage-fail→nothing, double-click→1 event, retryDelivery re-arm no re-render, worker bad-bearer→401).
...
- **SUBMIT-ORDER** `apps/web/src/app/api/translation/submit-order/route.ts`: new V2 contract `{checkout_id, canonical_document_id?, locale, doc_type, notes?}`. Stripe verify (paid+translation); recipient email ONLY from the verified session; client field values non-authoritative. Canonical (when present): `loadCanonicalDocumentById`→404, product!=translation→403, documentSessionId!=checkout→403, `verifyCanonicalHash`→409. Idempotent `createOrGetOrder` by checkout_session_id + `bindCanonicalDocument` + queued transition. Legacy `manual_review_queue` row STILL written (v2 linkage in event metadata). Gated by `getCanonicalMode('translation')` (off→legacy-only; shadow→V2 persisted; enforce→canonical_id required/422). Status matrix 422/402/403/404/409/503. PII-free logging.
- **OPERATOR AUTH** `apps/web/src/lib/auth/requireTranslationOperator.ts`: independent `admin_session` cookie===`ADMIN_SECRET` check at the mutation boundary (fail-closed 403 when unset; 401 absent/wrong), returns opaque actor `operator:admin`. `OperatorAuthError` typed 401/403.
- **V2 ACTIONS** `apps/web/src/app/admin/manual-review/[id]/v2Actions.ts`: assignOrder/beginReview/requestClarification/appendOverride/approveForRender/retryDelivery/cancelOrder/changeRecipient — each calls `requireTranslationOperator()` first, asserts expected version (`loadAtVersion`→ORDER_VERSION_CONFLICT/409) + state, performs one audited transition/override. Field edits via canonical override channel (`applyOperatorOverride`). Recipient change is a separate audited action (confirm+reason+old/new hash). approveForRender renders ONCE + `create_artifact_and_enqueue`, does NOT email.
- **LEGACY ACTIONS** `apps/web/src/app/admin/manual-review/[id]/actions.ts`: retrofitted `requireTranslationOperator()` into sendTranslation/approveAndSendPdf/markInReview (no unguarded Server Action mutation).
- **OPERATOR UI** `apps/web/src/app/admin/manual-review/[id]/v2/page.tsx`: loads V2 order + `resolveCanonicalDocument` + override history + state/version + verified recipient + prior artifacts + events; renders base/normalized/effective + review reasons + override input. Authority = resolved canonical, NOT mutable source_fields.
- **CANONICAL RENDER** `apps/web/src/lib/translation/orders/renderFromCanonical.ts`: resolve → map to renderer fields (resolved finalValue, NOT fabricated confidence=1/raw=edited), C3-null omitted (INV-11), render ONCE via `generateTranslationPDF`, compute 7-field certification (canonical_id, base/resolved/override_set hashes, override_version, canonical_schema_version, renderer_version) + byte SHA-256.
- **DELIVERY WORKER** `apps/web/src/app/api/internal/translation-delivery/route.ts`: CRON_SECRET Bearer auth; `claim_outbox_event` exactly-once; loads exact stored bytes via `downloadArtifactBytes` (hash-verified, NEVER re-renders); resend send with outbox idempotency_key; success→markOutboxDelivered+transition delivered; transient→markOutboxFailed (retry, exp backoff 1→16min); permanent→markOutboxPermanentlyFailed+delivery_failed. No recipient/content logged.
- **DATA MODULE** `apps/web/src/lib/translation/orders/index.ts`: added `getArtifactById`, `downloadArtifactBytes` (sha-verified), `markOutboxPermanentlyFailed`. **EMAIL** `lib/email/resend.ts`: optional `idempotencyKey` passed to Resend (duplicate retry = provider no-op).
- **TESTS** (37 new): `renderFromCanonical.test.ts` (override-in-resolved, base-unchanged, 7-cert, bytes==hash, C3-null-omitted), `submitOrderV2.test.ts` (client values ignored, binding, 403 product/session, 404, idempotent, status matrix, PII-free), `operatorAuthAndActions.test.ts` (401/403 auth, every action auths first, version/state checks, override channel, separate recipient action, legacy actions guarded), `deliveryWorker.test.ts` (CRON auth, exactly-once claim, no-regen, hash-verify, idempotency_key, backoff, PII-free).
- **GATE**: tsc 0 errors; full vitest 3727 pass / 38 skip / 0 fail; build PASS (new routes `/api/internal/translation-delivery`, `/admin/manual-review/[id]/v2`). Branch `architecture/translation-operator-canonical-v2`; NOT merged; no Vercel/env change; enforce NOT enabled; prod stays shadow.

## 2026-06-14 | feat(translation): Phase 2 Operator Pipeline V2 — DB foundation (orders, state machine, artifacts, outbox, security)

- **MIGRATIONS** (additive, forward-only; applied to prod `rtfxrlountkoegsseukx`):
  - `supabase/migrations/20260614000001_translation_orders_v2_and_state_machine.sql`: `translation_orders_v2` (durable order — checkout_session_id UNIQUE, optional `canonical_document_id` FK, product='translation' CHECK, status+version CHECKs, legacy flag, created/updated/paid/completed/expires timestamps; indexes). `translation_order_events` (append-only). `transition_translation_order(...)` RPC — the ONLY status/version mutator: advisory xact lock, status+version read FOR UPDATE, rejects status≠expected (ORDER_STATE_CONFLICT/P0002), version≠expected (ORDER_VERSION_CONFLICT/P0002), disallowed transition (ORDER_INVALID_TRANSITION/P0001), null actor (ORDER_ACTOR_REQUIRED), bumps version + appends event. `translation_order_transition_allowed(from,to)` immutable map (10 states). BEFORE UPDATE guard blocks any status/version change outside the RPC (txn-local GUC), blocks version decrement, checkout immutable, no canonical rebind. Append-only event triggers.
  - `supabase/migrations/20260614000002_translation_artifacts_outbox_and_security.sql`: widens `canonical_overrides_source_check` to admit 'operator_override' (additive). `document_artifacts` (immutable, hash-bound, UNIQUE(order_id,artifact_version), immutability triggers). `delivery_outbox` (idempotency_key UNIQUE, opaque recipient_ref). `create_artifact_and_enqueue` — ONE txn: artifact + transition approved_for_render→artifact_generated→delivery_pending + outbox (rollback→no orphan; email outside). `claim_outbox_event` — FOR UPDATE SKIP LOCKED (exactly-once). Private bucket `translation-artifacts`. `phase2_admin_cleanup(prefix)` — service-role-only sentinel cleanup.
  - `supabase/migrations/20260614000003_widen_canonical_guards_for_phase2_sentinel.sql`: CREATE OR REPLACE canonical immutability guards to also release for `PHASE2_TEST_%` sentinels (real rows stay immutable).
- **SECURITY**: 4 new tables RLS-enabled service_role-only (no anon/authenticated). All new functions SECURITY DEFINER + SET search_path + REVOKE PUBLIC/anon/authenticated + GRANT service_role. Immutability enforced by TRIGGERS (service_role bypasses RLS). Security advisor: 0 new Phase 2 findings.
- **NAMING**: legacy `public.translation_orders` (2 real rows) UNTOUCHED; new entity is `translation_orders_v2`.
- **TS MODULE** `apps/web/src/lib/translation/orders/index.ts`: createOrGetOrder/getOrderBy*/bindCanonicalDocument/transitionOrder/applyOperatorOverride (reuses canonical override channel, source='operator_override')/resolveOrderCanonical/createArtifactAndEnqueue/listOrderArtifacts/claimOutboxEvent/markOutboxDelivered/markOutboxFailed/phase2AdminCleanup; typed `TranslationOrderError`+`classifyOrderError`. No parallel translated_fields authority (documented).
- **TESTS**: `orderErrors.unit.test.ts` (7 unit). `phase2DataModel.live.test.ts` (14 live, RUN_DB_INVARIANTS-gated, synthetic PHASE2_TEST_, cleanup after): dup-submit→one order; no canonical rebind; per-order binding+FK; stale version; invalid transition; two-operators-one-conflict; artifact immutable; events immutable; atomic enqueue; rollback→no orphan; exactly-once claim; sha256 store/recompute; C3-null survives resolve; operator override changes effective value, base unchanged.
- **GATE**: tsc 0 errors; unit 7/7 PASS; live DB 14/14 PASS; 0 sentinel rows left. Foundation only — not wired; no prod env/flag change.

## 2026-06-13 | fix(canonical): legacy global can't enforce (product-scoped enforce only) + TPS positive carriage assert + PR cleanup
