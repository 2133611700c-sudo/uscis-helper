...
<!-- 2026-07-25 Resend relay cutover verified: Federal Register digest delivery moved from the conclusively invalid GitHub RESEND_API_KEY (job 89509758114) to the authenticated production relay deployed by PR #33. GitHub uses the existing Supabase service-role secret only as HMAC input; the raw secret is never transmitted and Vercel keeps ownership of RESEND_API_KEY/RESEND_FROM. Live run 30163334428 passed with delivery_status=sent and delivery_transport=relay. The temporary proof trigger was removed before review. -->
