...
<!-- 2026-07-25 Production reader fallback cutover: UI smoke 30163666660 proved production Gemini OCR returns HTTP 429 across its model chain and times out the wizard smoke. Preview then proved the existing resilience path end-to-end: Gemini primary failed, OpenAI gpt-4.1 succeeded, 10 birth-certificate fields returned, fallback_used=true. Set ONE_BRAIN_READER_FALLBACK=1 in tracked Vercel runtime config. Candidate ≠ Final remains enforced because fallback fields are candidate-only and force-reviewed. -->
