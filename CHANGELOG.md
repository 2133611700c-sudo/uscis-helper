...
## 2026-05-31 — Session 90: Live benchmark runner scaffold (branch feat/live-benchmark-runner)

Merged #65 (benchmark infra). Built the live-runner scaffold (no migration/flags/UI/payment). New `apps/web/src/lib/canonical/core/benchmark/liveRunner.ts` (runLiveBenchmark: injected real-reader callers → score → report + PII-free summary; BLOCKED if none ran). New `scripts/run-live-reader-benchmark.mjs` (CLI --image/--ground-truth/--doc-type; BLOCKED exit 2 on missing image/truth/GEMINI key; wires Gemini docintel + Core as real callers; MRZ/old-TPS = follow-ups; full PII report → qa-private/reports, sanitized → docs/reports/live-reader-benchmark). Data safety: .gitignore += qa-private/** (verified ignored); only sanitized summary committable; docs/reports/live-reader-benchmark/README + .gitkeep. New liveRunner.test.ts 4/4; .mjs BLOCKED exit-2 verified; full web 2381 pass; tsc 0; content-guard 0. NOT LIVE: one brain live=NO, real benchmark=NO, migration=NO. Needs owner real doc + ground truth + GEMINI_API_KEY_PAY. Migration = manual approval. Files: core/benchmark/liveRunner.ts, scripts/run-live-reader-benchmark.mjs, .gitignore, docs/reports/live-reader-benchmark/{README.md,.gitkeep}, core/__tests__/liveRunner.test.ts, HANDOFF/STATUS/CHANGELOG.
