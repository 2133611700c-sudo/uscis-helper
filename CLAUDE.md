...
- **Primary LLM reader = `gemini-3.1-pro-preview` ONLY** (acceptance-valid LLM). PREVIEW with NO capacity guarantee → sporadic 503/429; retried with exponential backoff before any fallback. (Per ADR-026 it is NOT the handwriting reader and is run-to-run unstable.)
