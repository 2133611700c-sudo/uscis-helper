...
- **⚠ CORRECTION (ADR-026, 2026-06-24) — ROUTE BY FIELD RENDERING.** Verified on the owner's real docs: a HANDWRITTEN field reads correctly **key-free** via `raxtemur/trocr-base-ru` on a NATIVE-resolution crop + contrast (never downscale/binarize) — it BEATS the LLMs on cursive but CANNOT abstain (fabricates on a blank crop) → its non-exact reads are gated + human-reviewed. A PRINTED field → an LLM (Gemini/GPT read print perfectly; raxtemur fails print). Earlier preview-primary experiments are historical only and were dropped from the active matrix. The handwriting reader + route-by-rendering are NOT yet wired into the production pipeline (sidecar hosting decision pending) — the bullets below describe the LLM availability matrix as currently shipped.
- **Primary LLM reader = `gemini-2.5-pro`** (stable GA; reads PRINTED Cyrillic correctly + stably across runs). Earlier preview-primary experiments were removed from the active matrix on 2026-06-24; treat them as history only. 2.5-pro stays DISQUALIFIED on handwritten certs (fabricates) → handwriting = raxtemur (ADR-026, not yet wired) + human review.
- **Fallbacks = AVAILABILITY only, force-reviewed, NEVER acceptance, NEVER primary**, in PREFERENCE order: `gemini-3.5-flash`, `gemini-2.5-flash` (code truth: `FALLBACK_MODELS` in `modelMatrix.ts`; the primary is NOT a fallback — an earlier revision of this bullet wrongly listed it in both roles). A fallback read of a non-Latin doc is force-reviewed (`fallback_model_used`).
