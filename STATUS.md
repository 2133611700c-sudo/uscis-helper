# STATUS (2026-06-12 — REAL-DOC verified: intl passport (SERGII/oblast/sex) + 4-page passport=0 fixed)
...
- INTERNATIONAL PASSPORT — all 3 bugs FIXED + verified on his real passport: (1) given_name SERGII (was SERHII) — bilingual `script:'mixed'` docs now prompt the model to return the printed LATIN romanization, and the policy keeps an already-Latin name VERBATIM (controlling-Latin rule, never re-transliterate СЕРГІЙ→Serhii). (2) sex Ч/М → Male (the 'sex' case now splits bilingual "Ч/M"). (3) place "ВІННИЦЬКА ОБЛ./UKR" → "Vinnytsia Oblast" (place_city strips the /UKR country code and routes obl./область to the oblast normalizer; JS \b doesn't work on Cyrillic — matched directly). Final: KUROPIATNYK / SERGII / PASSPORT-REDACTED / 1986-06-25 / Male / Vinnytsia Oblast / 2019-02-22 / 2029-02-22 — all correct.
- 4-PAGE PASSPORT "вообще 0" FIXED + verified: the deadline regression (each page capped at 40s was too tight) is resolved — page 1 of the real 4-page booklet now reads in 14s (Kuropiatnyk/Serhii/Trostianets/Vinnytsia Oblast). Route Core read: timeoutMs 85s TOTAL deadline + attemptsPerModel:1 (slow primary doesn't burn the budget; falls back to flash). Pages 3-4 honestly return little/0 (no fabrication). The cross-page arbiter takes the earliest (correct) page.
