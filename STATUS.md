...
- NEW TESTS (all behavioral, drive REAL product code; NO external Stripe / NO live DB / NO prod bypass):
  - `api/stripe/webhook/__tests__/webhookPaymentBoundary.test.ts` (8): missing/invalid signature→400, missing-secret→400 fail-closed, wrong event type→no transition, payment_failed→never paid, verified completed→audit + legacy update gated status='signed'+email, no-email/wrong-product→no order update. Mocks the Stripe SDK `constructEvent` boundary IN TEST ONLY.
  - `lib/translation/orders/__tests__/operatorPipelineBehavioral.test.ts` (29) + `__tests__/fakeOrdersDb.ts`: drives `orders/index.ts` + `v2Actions` + delivery worker against a faithful in-process fake of the Phase 2 Supabase RPCs/triggers. Wave 3A (idempotent order, server-only recipient, immutable canonical, version+audit, actor-required, atomic artifact+outbox, INV-11 omit, no-dup-outbox) + 3B (authz 401/403/409, override provenance, exactly-once delivery, hash-verify, no re-render, concurrent-claim one-winner, tamper→retry, transport-fail→retryable, crash-no-dup, missing-recipient→permanent) + CHAOS (rollback no-orphan, storage-fail, double-click 1-event, retry-no-rerender, bad-bearer 401).
  - `lib/translation/orders/__tests__/phase2OrdersInvariants.live.test.ts` (8, SELF-SKIPS unless `RUN_DB_INVARIANTS=1`): same invariants vs REAL Supabase, `PHASE2_TEST_` sentinels, guarded cleanup.
- EMAIL SEAM: `lib/email/resend.ts` now injectable (`EmailTransport` + `setEmailTransportForTesting`/`resetEmailTransport`); prod default = real Resend, UNCHANGED; tests inject a capturing transport.
- GATE: input_sha=44ec7b8…; tsc 0 errors; full vitest 3764 pass / 46 skip / 0 fail (+65 new pass, +8 new live-skip). Hosted Stripe POSITIVE E2E owner-DEFERRED to final acceptance. Prod SHADOW, PR #119 DRAFT, no merge, no env/flag change.

---
# STATUS (2026-06-13 — PHASE 2 AGENT 4: observability/operations layer ADDED on branch)
- OBSERVABILITY/OPS layer added onto Phase 2 (branch `architecture/translation-operator-canonical-v2`, base 44ec7b8). ADDITIVE, no behavior change, prod stays SHADOW.
- EVENTS (`apps/web/src/lib/translation/observability/events.ts`): typed PII-safe emitter (thin `console.info` wrapper, silent unless `PHASE2_EVENTS_ENABLED=1`). 25 event codes (orders/operator/artifact/delivery/payment_boundary). The dimension type makes PII unrepresentable; `assertPiiSafe()` rejects any forbidden/unknown key, email-like value, or over-long free text. Wired additively into submit-order (orders_created/transition_failures), v2Actions (override/artifact gen+storage fail/auth-denied/stale-version), delivery worker (attempts/success/failure/duplicate-prevented/hash-mismatch/claim-fail), stripe webhook (received/signature-failure).
- ALERTS (`docs/observability/PHASE2_ALERTS.md`): 10 thresholds + auto-responses incl. artifact hash mismatch>0 CRITICAL, outbox age, delivery failure spike, operator queue age, webhook signature spike, paid-no-order, ready-no-artifact.
- LIFECYCLE/RETENTION (`apps/web/src/lib/translation/lifecycle.ts` + `docs/observability/PHASE2_RETENTION.md`): pure config + planners (NO destructive migration). Retention windows, signed-URL TTL (short-lived, private, non-reusable after expiry, hard cap 1h), customer-deletion plan (no orphan storage, preserve non-PII audit stub), legal hold, expired-artifact access.
- RUNBOOKS (`docs/runbooks/01..08`): paid-no-order, webhook dup/delay, operator-cannot-open, artifact-gen-failed, hash-mismatch (quarantine+new-version-never-overwrite), email-delivery-failed (retry outbox, no PDF regen), wrong-recipient (Stripe-derived only), canonical-correction (new version+supersession).
- RELEASE_STATE.yaml at repo root (machine-readable; main_sha, per-product mode/carriage/positive_e2e, blockers). production_sha marked UNVERIFIED (not readable from worktree).
- AUDIT (Agent 1-3): no data-loss/dup-delivery/authz-bypass/race/PII findings that block. Transition GUC reset is txn-local + rollback-safe; claim_outbox_event SKIP LOCKED + UNIQUE idempotency_key = exactly-once; artifact immutability triggers + content-addressed storage prevent overwrite. Minor advisories recorded.
- GATE: tsc 0 errors; full vitest 3764 pass / 38 skip / 0 fail (37 NEW across 2 files: events PII-gate, lifecycle). NOT merged, no Vercel/env change, PHASE2_EVENTS_ENABLED OFF, prod shadow.

---
# STATUS (2026-06-14 — PHASE 2 AGENT 2: Translation Operator Pipeline V2 application flow WIRED on branch)
- APPLICATION FLOW wired onto Agent 1's DB foundation (branch `architecture/translation-operator-canonical-v2`). ADDITIVE; legacy operator path kept. Gated by `getCanonicalMode('translation')` (prod=shadow; V2 persists order + canonical binding while legacy output still shows).
- SUBMIT-ORDER (`apps/web/src/app/api/translation/submit-order/route.ts`): new contract `{checkout_id, canonical_document_id?, locale, doc_type, notes?}`. Client field VALUES are NOT authoritative. Stripe verify (paid+translation); recipient email ONLY from the verified session. Canonical (when present): loadById→404 / product!=translation→403 / session-ownership→403 / verifyCanonicalHash→409. Idempotent `createOrGetOrder` by checkout_session_id; binds canonical + verified email; enqueues queued; STILL writes legacy `manual_review_queue` row (marks v2 linkage). enforce→canonical required(422). Status matrix: 422/402/403/404/409/503.
- OPERATOR AUTH (`apps/web/src/lib/auth/requireTranslationOperator.ts`): independent admin_session-cookie===ADMIN_SECRET check (fail-closed 403 if unset, 401 if absent/wrong) at the MUTATION boundary, returns opaque actor `operator:admin`. Called FIRST in every V2 action (`[id]/v2Actions.ts`: assignOrder/beginReview/requestClarification/appendOverride/approveForRender/retryDelivery/cancelOrder/changeRecipient) AND retrofitted into the 3 legacy actions (`[id]/actions.ts`). Each action asserts expected version+status; recipient change is a SEPARATE audited action (confirm+reason+old-hash).
- OPERATOR UI (`[id]/v2/page.tsx`): loads V2 order + resolveCanonicalDocument + override history + state/version + verified recipient + prior artifacts + events; shows base/normalized/effective + review reasons + override input. Uses resolved canonical as authority, NOT mutable source_fields.
- RENDER (`lib/translation/orders/renderFromCanonical.ts`): resolveCanonicalDocument → render PDF ONCE via existing renderer (resolved finalValue, NOT fabricated confidence=1); C3-null omitted (INV-11). 7-field certification binding (canonical_id, base/resolved/override_set hashes, override_version, schema_version, renderer_version). SHA-256 of bytes; upload to private `translation-artifacts`; `create_artifact_and_enqueue` (atomic). approveForRender does NOT email.
- DELIVERY WORKER (`api/internal/translation-delivery/route.ts`): CRON_SECRET Bearer auth; `claim_outbox_event` (exactly-once SKIP LOCKED); loads EXACT stored bytes (hash-verified, NEVER re-renders); sends via resend with the outbox idempotency_key; success→delivered+transition; transient→retry+exp backoff; permanent→delivery_failed. No recipient/content logged.
- GATE: tsc 0 errors; full vitest 3727 pass / 38 skip / 0 fail (new: 37 across 4 files); build PASS (new routes `/api/internal/translation-delivery`, `/admin/manual-review/[id]/v2`). NOT merged, no Vercel/env change, enforce NOT enabled, prod stays shadow.

---
# STATUS (2026-06-14 — PHASE 2 AGENT 1: Translation Operator Pipeline V2 DB foundation LIVE on prod)
- NEW DB FOUNDATION (branch `architecture/translation-operator-canonical-v2`, 3 additive forward-only migrations applied to PROD `rtfxrlountkoegsseukx`): durable order aggregate `translation_orders_v2`, append-only `translation_order_events`, immutable `document_artifacts`, transactional `delivery_outbox`. NAMING: legacy `translation_orders` (2 real rows) is UNTOUCHED — new entity is `_v2` to honor the no-alter-existing constraint.
- STATE MACHINE: `transition_translation_order(order,expected_version,expected_status,to_status,actor,reason,metadata)` is the ONLY status/version mutator (advisory-locked optimistic concurrency → ORDER_STATE_CONFLICT / ORDER_VERSION_CONFLICT / ORDER_INVALID_TRANSITION / ORDER_ACTOR_REQUIRED). A BEFORE UPDATE trigger blocks any direct status/version change (service_role bypasses RLS → trigger, not policy, via txn-local GUC `translation_orders.allow_transition`). 10 states, in-function allowed-transition map.
- ATOMICITY: `create_artifact_and_enqueue` does artifact INSERT + 2 transitions + outbox INSERT in ONE txn (rollback → no orphan outbox). `claim_outbox_event` claims one due row FOR UPDATE SKIP LOCKED (exactly-once; duplicate `idempotency_key` UNIQUE-rejected). Email send is OUTSIDE the txn.
- OPERATOR OVERRIDES: REUSE `canonical_overrides` via `append_canonical_overrides_atomic` (source='operator_override', confirmed=true, actor=operator) — resolve through `resolveCanonicalDocument`. NO parallel mutable translated_fields authority. `canonical_overrides_source_check` widened additively to admit 'operator_override' (superset; no row invalidated).
- SECURITY: all 4 new tables RLS-enabled, service_role-only, no anon/authenticated policy. All new functions SECURITY DEFINER + SET search_path + schema-qualified + REVOKE PUBLIC/anon/authenticated + GRANT service_role. Private storage bucket `translation-artifacts` (public=false). Security advisors: ZERO new findings on Phase 2 objects.
- TS MODULE `apps/web/src/lib/translation/orders/index.ts` (typed access: createOrGetOrder/transitionOrder/applyOperatorOverride/resolveOrderCanonical/createArtifactAndEnqueue/claimOutboxEvent/phase2AdminCleanup; typed `TranslationOrderError`).
- GATE: tsc 0 errors; unit 7/7 pass; LIVE DB invariants 14/14 pass (synthetic PHASE2_TEST_ data, cleaned to 0). NOT wired into any route yet (foundation only). NO prod env/flag change.
