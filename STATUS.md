...
- **Measured model truth (same fixture):**
  ...
  - `gpt-4.1` pipeline bench: birth cert `0/5`, `samePerson=NO`; raw bench `CYR 0/3`, `LAT 0/3`, `DOB N`.
  - `gpt-4o` pipeline bench: birth cert `0/5`, `samePerson=NO`; raw bench `MODEL_REFUSAL` on military + birth.
  - `gpt-5` raw bench: passport partially okay, military/birth `0/3`, `DOB N`, ~42-105s.
  - `gpt-5.5-pro` current API path = `HTTP 404`, not a callable candidate.
- **Verdict:** no direct GPT/Gemini image-reader path is acceptable as the handwritten-birth-certificate primary. Correct direction remains HTR / field-crop / human-review. DeepSeek remains text-only structuring, not image OCR. Report: `docs/reports/HANDWRITTEN_BIRTH_CERT_MODEL_PROTOCOL_2026-06-24.md`.

<!-- ROOT-CAUSE REVERSAL (2026-06-24): 4 parallel agents + independent repro OVERTURNED "no key-free model reads the surname". raxtemur/trocr-base-ru (Apache, local, NO keys) reads child name EXACTLY given NATIVE-res crop from 4128x3096 + contrast-stretch (NO downscale/NO binarize): surname CER 0.000 (3/3), given 0.000, patronymic 0.333 match. IMPORTANT correction: blank-control was NOT clean — raxtemur fabricates non-empty text on blank input, so review stays mandatory. Binding root = field crop was downscaled to h128 (signal destroyed); + scorer Latin-vs-Cyrillic channel bug; + label/stamp contamination. Data NOT the constraint; synthetic fine-tune abandoned. Standard: ADR-026 (native-res field crop + contrast only, never downscale/binarize, channel-aware scoring, raxtemur reader, human-review stays). Corrected MODEL_INVENTORY + HTR_ZEROSHOT_POC. Scope = modern UA & RU Cyrillic. NEXT: wire native-res field reader into apps/web pipeline + auto field-region localization + re-test LLM APIs on native crops + broader-N validation. verify via qa-private/htr-poc/verify_root.py. -->

<!-- HTR ZERO-SHOT POC (2026-06-24): key-free open Cyrillic HTR baseline on the real birth cert (gitignored qa-private/htr-venv, torch/transformers, NO API keys). raxtemur/trocr-base-ru (Apache) + cyrillic-trocr (MIT) READ the child GIVEN name the LLM APIs fabricate; NONE read the cursive SURNAME zero-shot (CER>1; apostrophe + stamp over ink); Kansallisarkisto (Apache) missed both. => off-the-shelf zero-shot NOT autonomous for the critical field; lever = fine-tune ~30-50 labeled pages (CER ~8-12%), labeling cost not money/keys. Production unchanged: handwritten=human review, printed=LLM APIs. Report docs/research/HTR_ZEROSHOT_POC.md (PII-free); landscape docs/research/CYRILLIC_HTR_LANDSCAPE.md. OWNER DECISION pending on the fine-tune path. BAKE-OFF COMPLETE (4 local candidates): added PyLaia CRNN-CTC UA+CS (Apache, polyscriptor MIT loader) + stamp-suppression — UA-CRNN surname CER 0.90/conf 0.33, given worse than TrOCR; CS transfer-gap confirmed; stamp only ~5% of crop (removal 1.10→0.70 partial) => blocker is the cursive, not occlusion. DECISIVE: no key-free model (TrOCR/PyLaia/UA-specific) reads the cursive surname zero-shot. Only key-free lever left = fine-tune on OUR labeled UA/Soviet corpus (PyLaia/Kraken CTC). Cloud readers NOT run (PII egress). -->
...
<!-- PLACE RECOGNITION ROOT FIXES (2026-06-22, branch translation/ru-and-model-matrix-fixes): (1) М-place bug — normalizePlace (packages/knowledge/normalize.ts) matched the bare SETTLEMENT_TYPES key «м» via startsWith, releasing «МОРИНЦІ» as "city ORYNTSI" (latent corruption of every М-/С-initial place: Миколаїв/Маріуполь/Мелітополь/Мукачево). FIX: designator must be a whole token (dotted abbr or followed by ws/dot); never split a name beginning with М/С. (2) Foreign-country dictionary — added COUNTRIES(24)/lookupCountry/normalizeForeignPlace to dictionary.ts; knowledgeNormalize place branch tries foreign FIRST → «Канада, місто Торонто»→"Canada, Toronto" (was "Kanada, misto Toronto"); null for domestic so UA path untouched. (3) Synthetic fixtures de-famoused — replaced real Taras Shevchenko (Gemini fabricated his real birthplace Моринці instead of reading the text) with invented ЮРЧЕНКО ОЛЕКСІЙ ВАСИЛЬОВИЧ / смт Лісове; e2e + workflow comment updated; 5 PNGs render. Integrated SURGICALLY (git-apply hunks) since both fix-agents branched off an older base — patronymicRu/KMU_RU_FALLBACK/noCyrillicLeak/CIVIL_STATUS verified intact. EVIDENCE: knowledge 258/0, canonical-place 59/0, knowledgeNormalize 18/0, tsc apps/web 0. No live Gemini this session (deterministic fixes). NEXT: dispatch staging-e2e-translation on this branch to confirm on a live read (verify headSha). -->
...
<!-- 2026-06-29: one-brain branch truth reverified at HEAD 1a3267a (local, unpushed). REAL current status after critical audit: full `pnpm --filter web test` = 5074 passed / 26 skipped (the prior branch-local timeout instability was closed by raising per-file timeouts in piiGuard.test.ts + knowledgeSafetyNet.test.ts); `pnpm --filter web run typecheck` = PASS; `pnpm --filter web run build` = PASS with existing Next/Sentry/libheif warnings only. Translation core cutover is real and template-evidence is already wired in backend behind ONE_BRAIN_EVIDENCE_ENABLED, so older branch docs saying bbox/evidence is "not called" were stale and corrected. Also corrected stale observability terminology: EAD/ReParole OCR routes now label the real cutover gate as ONE_BRAIN_RECOGNIZE_ENABLED instead of legacy ONE_CORE_* names. Still true: branch has NO upstream / NO CI proof / NOT live in prod. -->
<!-- 2026-07-04: transliteration audit — KMU-55 + Russian source-script routing verified by direct import; mixed-script guard catches mixed tokens; `node scripts/check-no-pii.mjs` clean. Full pnpm/vitest runner blocked by workspace relink EPERM / missing local binaries, so verdict stays TRANSLITERATION_SPOT_PASS / RUNNER_BLOCKED. -->
...
<!-- 2026-07-05: orientation follow-up — handwritten doc classes now run the 180° confirm by default even without `ORIENT_180_CHECK=1`. Live probe on the real handwritten fixtures showed the prior 180° miscorrections on `birth_cert_handwritten_01` and `military_id_p1_01` now resolve correctly; printed/sparse fixtures remain unchanged. Targeted vitest, `tsc --noEmit`, and `node scripts/check-no-pii.mjs` are green. -->
<!-- 2026-07-06: new one-brain probe tool — added `apps/web/scripts/one-brain-orient-read.mts` plus canonical wrapper `apps/web/src/lib/docintel/oneBrainTool.ts`. Live probes on `birth_cert_handwritten_01.jpg` and `military_id_p1_01.jpg` both resolved orientation to upright via content_orient cw=90; Gemini hit HTTP 429 in this shell, but explicit `--provider openai` fallback read both docs successfully. Verified by targeted vitest, `tsc --noEmit`, and `node scripts/check-no-pii.mjs`. -->
<!-- 2026-07-06: tool alias follow-up — `one-brain-orient-read.mts` now accepts common file-name aliases such as `internal_passport_01` → `ua_internal_passport_booklet`. Live probe on that class still returns `ok` with 0 handwritten fields under the openai fallback, so aliasing is fixed but the reader remains empty on that fixture. -->
