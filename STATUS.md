# STATUS — Messenginfo
## Session 90 (2026-05-30) — Live benchmark runner scaffold (branch feat/live-benchmark-runner; #65 merged)
- `BENCH` `liveRunner.ts` (`runLiveBenchmark` — injected real-reader callers → score → report + PII-free summary; BLOCKED if none ran) + `scripts/run-live-reader-benchmark.mjs` (CLI `--image --ground-truth --doc-type`; BLOCKED exit 2 on missing image/truth/GEMINI key; writes full PII report → qa-private/reports, sanitized → docs/reports/live-reader-benchmark). Real callers wired: Gemini docintel + Core; MRZ/old-TPS = honest follow-ups (need locateMrzLines export / dev endpoint).
- `SAFETY` `.gitignore += qa-private/**` (real docs/ground-truth/full PII reports never committed — verified ignored); only sanitized summary committable.
- `EVIDENCE` `liveRunner.test.ts` 4/4 + .mjs BLOCKED exit-2 verified. Full web 2381 pass, tsc 0, guard 0. #65 merged. ONE BRAIN LIVE = NO, REAL BENCHMARK = NO, MIGRATION = NO. Needs owner real doc + ground truth + GEMINI_API_KEY_PAY to produce numbers. Migration = manual approval. PR not merged.
