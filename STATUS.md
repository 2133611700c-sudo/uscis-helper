# STATUS — Messenginfo
## Session 91 (2026-05-31) — PROD HOTFIX: Gemini key name + recognition root-cause (branch fix/gemini-key-066-name)
- `ROOT CAUSE (verified)` Prod `/api/translation/vision-extract` → 502 on real docs. Code reads ONLY `GEMINI_API_KEY_PAY`/`GEMINI_API_KEY` (vision-extract:109, geminiVisionProvider:99); owner uploaded the working key to Vercel under name **`GEMINI_API_KEY_066`** → app never sees it → uses dead key → central-brain consensus reads 0 fields → route returns 502 by design (line 130). = "ничего не распознаётся".
- `FIX` New `lib/gemini/apiKey.ts` `getGeminiApiKey()` reads the key from ANY `GEMINI_API_KEY*` env name (owner kept renaming: 066 → GEMINI_API_KEY2 …) — suffixed names preferred over bare. Wired into vision-extract + geminiVisionProvider. VERIFIED end-to-end: with var named `GEMINI_API_KEY2`, readDocument reads the real booklet (REDACTED/SERHII/1986-06-25/Vinnytsia, ok=true). apiKey.test 6/6, tsc 0, full web 2383 pass, guard 0.
- `PROVEN` With the working key, single Gemini read (`docintel.readDocument`) reads BOTH real docs locally: internal booklet (REDACTED/SERHII/1986-06-25/Vinnytsia, 25s) + birth cert (10 fields, 8.6s). Single read WORKS where central-brain consensus returns 0→502. Confirms the one-brain single-read approach fixes prod.
- `ALSO FOUND (real-data bugs for Core)` (1) reader field-key vocabulary mismatch (`dob` vs `date_of_birth`; birth-cert keys) → Core mis-criticalizes; (2) reader `review_required` not carried into Core candidate → Core under-flags. Both grounded in real run, to fix next.
- `502→DEGRADE` Also fixed `route.ts`: when central-brain consensus reads 0 fields it now DEGRADES to the legacy single-read path (proven to read real docs) instead of returning 502. So merging #67 alone makes recognition work even if central-brain yields nothing.
- `INSTANT OWNER OPTION` (without merge) rename Vercel var to a `GEMINI_API_KEY*` the code reads — but #67 now reads `GEMINI_API_KEY2` directly, so just merge #67. Manual approval; PR not merged by me.
